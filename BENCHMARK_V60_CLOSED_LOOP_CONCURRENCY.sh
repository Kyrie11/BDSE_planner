...
GPUS="${GPUS:-0,1}"
BENCH_ROOT="${BENCH_ROOT:-outputs_v60_closed_loop_concurrency_benchmark}"
...
for p in $PROCESS_SETTINGS; do
  ...
  V60_CKPT="$CANDIDATE_CKPT" EVAL_CONFIG="$CANDIDATE_CONFIG" \
  bash run_v60_dwapc_bfar_dbap.sh
  stop=$(date +%s)
  python - "$BENCH_ROOT/benchmark_summary.json" "$p" "$((stop-start))" "$out/closed_loop/v60_dwapc_bfar_dbap_4/closed_loop_combined_summary.json" <<'PY'
import json,sys
from pathlib import Path
