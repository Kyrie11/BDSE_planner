...
: "${CET_BDMU_SCREEN_REPORT:?Set CET_BDMU_SCREEN_REPORT to the promoted V64.3.12 screen report}"
for p in "$DARM_DBR_CKPT" "$DARM_DBR_FULL_REPORT" "$CET_BDMU_SCREEN_REPORT"; do
  [[ -s "$p" ]] || { echo "missing required artifact: $p" >&2; exit 2; }
done
python - "$DARM_DBR_FULL_REPORT" "$CET_BDMU_SCREEN_REPORT" <<'PY'
import json,sys
for name,p in [('DARM+DBR full',sys.argv[1]),('CET-BDMU screen',sys.argv[2])]:
    r=json.load(open(p))
    if not r.get('full_promotion', False):
        raise SystemExit(f'{name} did not meet promotion criteria; CET-BDMU full training blocked.')
PY
...
export TRAIN_CONFIG="bdse/configs/v64_3_12_cc_aocc_cet_bdmu_daepc_train_2gpu.yaml"
export EVAL_CONFIG="bdse/configs/v64_3_12_cc_aocc_cet_bdmu_cl.yaml"
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_12_cet_bdmu_full_2gpu_v1}"
mkdir -p "$OUT_ROOT/provenance" "$OUT_ROOT/logs"

python -m bdse.tools.validate_v64_pipeline_config \
  --train-config "$TRAIN_CONFIG" --eval-config "$EVAL_CONFIG" --expected-family v64.3.12 \
  --output "$OUT_ROOT/provenance/config_contract.json"
python -m bdse.tools.check_v64_3_12_cet_bdmu_contract \
  --config "$TRAIN_CONFIG" \
  --output "$OUT_ROOT/provenance/cet_bdmu_contract.json"
...
python -m bdse.tools.check_v64_3_12_ret_cet_bdmu_screen \
  --train-log "$OUT_ROOT/train/bdse_v64_saqa_bcc.train_log.jsonl" \
  --output "$OUT_ROOT/provenance/v64_3_12_cet_bdmu_full_audit.json" \
  --variant cet
echo "[v64.3.12] full report: $OUT_ROOT/provenance/v64_3_12_cet_bdmu_full_audit.json"
