...
# Accept both `SKIP_TRAIN=1 bash run_v27_trbsr.sh` and
# `bash run_v27_trbsr.sh SKIP_TRAIN=1` to avoid accidental training.
for kv in "$@"; do
  if [[ "$kv" == *=* ]]; then export "$kv"; fi
done


# v27 / T-RBSR: Risk-Bounded Safety Recovery
...
TRAIN_OUTPUT="$TRAIN_ROOT/bdse_v27_trbsr.pt"
V27_CKPT=${V27_CKPT:-$TRAIN_ROOT/bdse_v27_trbsr.best.pt}
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  echo "[train] v27 T-RBSR finetune from $WARM_START"
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    --config bdse/configs/v27_bdse_trbsr_train.yaml \
    --split train \
  ...
  echo "[train] SKIP_TRAIN=1, using existing checkpoint: $V27_CKPT"
fi

# For zero-training ablation, reuse the v25/v26 checkpoint with v27 runtime guard/configs:
#   SKIP_TRAIN=1 V27_CKPT=outputs_v25/train/bdse_v25_dgcace.best.pt bash run_v27_trbsr.sh
if [[ ! -f "$V27_CKPT" ]]; then
  echo "Missing v27 checkpoint: $V27_CKPT" >&2
  echo "For runtime-only check, run: SKIP_TRAIN=1 V27_CKPT=$V25_CKPT_IN bash run_v27_trbsr.sh" >&2
  exit 1
fi
...
paths = [
    ('v27_trbsr_fixed_budget', root / 'open_loop_v27_trbsr_fixed_budget.json'),
    ('v27_trbsr_safety_fallback', root / 'open_loop_v27_trbsr_safety_fallback.json'),
    ('v27_trbsr_bbr_scur', root / 'open_loop_v27_trbsr_bbr_scur.json'),
    ('v27_trbsr_lcb_control', root / 'open_loop_v27_trbsr_lcb_control.json'),
]
keys = [
...
run_open_loop 0 v27_trbsr_fixed_budget bdse/configs/v27_bdse_trbsr_fixed_budget_fast_cl.yaml "$V27_CKPT" &
run_open_loop 1 v27_trbsr_safety_fallback bdse/configs/v27_bdse_trbsr_safety_fallback_fast_cl.yaml "$V27_CKPT" &
wait
run_open_loop 0 v27_trbsr_bbr_scur bdse/configs/v27_bdse_trbsr_bbr_scur_fast_cl.yaml "$V27_CKPT" &
run_open_loop 1 v27_trbsr_lcb_control bdse/configs/v27_bdse_trbsr_lcb_control_fast_cl.yaml "$V27_CKPT" &
wait
print_open_loop_compare
...
run_closed_loop 0 v27_trbsr_fixed_budget bdse/configs/v27_bdse_trbsr_fixed_budget_fast_cl.yaml "$V27_CKPT" 20 &
run_closed_loop 1 v27_trbsr_safety_fallback bdse/configs/v27_bdse_trbsr_safety_fallback_fast_cl.yaml "$V27_CKPT" 20 &
wait
run_closed_loop 0 v27_trbsr_bbr_scur bdse/configs/v27_bdse_trbsr_bbr_scur_fast_cl.yaml "$V27_CKPT" 20 &
run_closed_loop 1 v27_trbsr_lcb_control bdse/configs/v27_bdse_trbsr_lcb_control_fast_cl.yaml "$V27_CKPT" 20 &
wait

python -m bdse.tools.collect_closed_loop_metrics \
  "$CL_ROOT/v27_trbsr_fixed_budget_20" \
  "$CL_ROOT/v27_trbsr_safety_fallback_20" \
  "$CL_ROOT/v27_trbsr_bbr_scur_20" \
  "$CL_ROOT/v27_trbsr_lcb_control_20" \
  --csv "$CL_ROOT/v27_20_compare.csv"
...
if [[ "${RUN_CL50:-0}" == "1" ]]; then
  run_closed_loop 0 v27_trbsr_fixed_budget bdse/configs/v27_bdse_trbsr_fixed_budget_fast_cl.yaml "$V27_CKPT" 50 &
  run_closed_loop 1 v27_trbsr_safety_fallback bdse/configs/v27_bdse_trbsr_safety_fallback_fast_cl.yaml "$V27_CKPT" 50 &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v27_trbsr_fixed_budget_50" \
    "$CL_ROOT/v27_trbsr_safety_fallback_50" \
    --csv "$CL_ROOT/v27_50_compare.csv"
fi

echo "Done. Key outputs:"
echo "  $TRAIN_ROOT/bdse_v27_trbsr.best.pt"
echo "  $OPEN_ROOT/open_loop_v27_*.json"
echo "  $CL_ROOT/v27_20_compare.csv"
echo "  $LOG_ROOT/*.closed_loop_20.diag.jsonl"
echo "Runtime-only first check: SKIP_TRAIN=1 V27_CKPT=$V25_CKPT_IN OPEN_LOOP_ONLY=1 bash run_v27_trbsr.sh"
echo "Runtime-only CL20: SKIP_TRAIN=1 V27_CKPT=$V25_CKPT_IN bash run_v27_trbsr.sh"
echo "Finetune: TRAIN_MAX_SCENARIOS=12000 VAL_MAX_SCENARIOS=1000 TRAIN_EPOCHS=4 bash run_v27_trbsr.sh"
