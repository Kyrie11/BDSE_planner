...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v39_dacc_runtime_v30ckpt}
OUT_ROOT="$(python -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$OUT_ROOT")"
OPEN_ROOT="$OUT_ROOT/open_loop"; CL_ROOT="$OUT_ROOT/closed_loop"; TRAIN_ROOT="$OUT_ROOT/train"; LOG_ROOT="$OUT_ROOT/v39_logs"
mkdir -p "$OPEN_ROOT" "$CL_ROOT" "$TRAIN_ROOT" "$LOG_ROOT"
RUN_MODE=${RUN_MODE:-open_loop}
V39_CKPT=${V39_CKPT:-$V30_CKPT_IN}
...
run_open_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_open_loop \
      --config "$cfg" --checkpoint "$V39_CKPT" --split val \
...
run_open_loop() {
  [[ -f "$V39_CKPT" ]] || { echo "Missing checkpoint: $V39_CKPT" >&2; exit 2; }
  run_open_loop_one "${GPU_OL_DACC:-0}" v39_dacc bdse/configs/v39_bdse_dacc_fast_cl.yaml &
  run_open_loop_one "${GPU_OL_CONTROL:-1}" v39_mars_control bdse/configs/v39_bdse_mars_control_fast_cl.yaml &
  wait
  set +e
  python -m bdse.tools.check_v39_dacc_gate \
    "$OPEN_ROOT/open_loop_v39_dacc.json" \
    "$OPEN_ROOT/open_loop_v39_mars_control.json" \
  ...
  if [[ "$status" -ne 0 && "${ENFORCE_RUNTIME_GATE:-1}" == "1" ]]; then
    echo "V39 runtime gate failed; do not run closed loop or finetune." >&2
    exit "$status"
  fi
}
...
run_train() {
  ...
  # Controlled v38-head finetune with the v39 NameError hotfix. DACC itself is
  # runtime-only in this version; validate runtime DACC before invoking train.
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --warm-start-from "$V30_CKPT_IN" --output "$TRAIN_ROOT/bdse_v39_hotfix_finetune.pt" --amp \
    --log-file "$LOG_ROOT/v39_finetune.jsonl" > "$LOG_ROOT/v39_finetune.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V39_CKPT" --device cuda \
...
run_closed_loop() {
  local limit="$1"
  [[ -f "$V39_CKPT" ]] || { echo "Missing checkpoint: $V39_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_DACC:-0}" v39_dacc bdse/configs/v39_bdse_dacc_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_CONTROL:-1}" v39_mars_control bdse/configs/v39_bdse_mars_control_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_FALLBACK:-0}" v39_dacc_fallback bdse/configs/v39_bdse_dacc_fallback_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v39_dacc_${limit}" "$CL_ROOT/v39_mars_control_${limit}" "$CL_ROOT/v39_dacc_fallback_${limit}" \
    --csv "$CL_ROOT/v39_${limit}_compare.csv"
}
