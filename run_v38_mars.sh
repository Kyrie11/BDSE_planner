...
# v38 MARS-BDSE: Margin-Aligned Residual Sparsification.
# Hard constraints are complete and budget-exempt; their continuous residual is
# compressed into J0.  B=16 selects decision evidence including soft feasibility.
ROOT_DIR="$(pwd)"
...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v38_runtime_v30ckpt}
...
LOG_ROOT="$OUT_ROOT/v38_logs"
mkdir -p "$TRAIN_ROOT" "$OPEN_ROOT" "$CL_ROOT" "$LOG_ROOT"

RUN_MODE=${RUN_MODE:-open_loop}
V38_CKPT=${V38_CKPT:-$V30_CKPT_IN}
V38_MAIN_CFG=${V38_MAIN_CFG:-bdse/configs/v38_bdse_mars_balanced_fast_cl.yaml}
V35_BASELINE_JSON=${V35_BASELINE_JSON:-$ROOT_DIR/outputs_v35_runtime_v30ckpt/open_loop/open_loop_v35_dice_hard7.json}
V35_BASELINE_JSONL=${V35_BASELINE_JSONL:-$ROOT_DIR/outputs_v35_runtime_v30ckpt/open_loop/open_loop_v35_dice_hard7.jsonl}
...
run_runtime_gate() {
  [[ -f "$V38_CKPT" ]] || { echo "Missing checkpoint: $V38_CKPT" >&2; exit 2; }
  run_open_loop "${GPU_OL_BALANCED:-0}" v38_mars_balanced bdse/configs/v38_bdse_mars_balanced_fast_cl.yaml &
  run_open_loop "${GPU_OL_RISK:-0}" v38_mars_winner_strong bdse/configs/v38_bdse_mars_winner_strong_fast_cl.yaml &
  run_open_loop "${GPU_OL_FULL:-1}" v38_mars_pair_only bdse/configs/v38_bdse_mars_pair_only_fast_cl.yaml &
  run_open_loop "${GPU_OL_INTERACTION:-1}" v38_mars_actionrank_control bdse/configs/v38_bdse_mars_actionrank_control_fast_cl.yaml &
  wait

  set +e
  baseline_args=()
  [[ -f "$V35_BASELINE_JSON" ]] && baseline_args+=(--baseline "$V35_BASELINE_JSON")
  [[ -f "$V35_BASELINE_JSONL" ]] && baseline_args+=(--baseline-jsonl "$V35_BASELINE_JSONL")
  python -m bdse.tools.check_v38_runtime_gate \
    "$OPEN_ROOT/open_loop_v38_mars_balanced.json" \
    "$OPEN_ROOT/open_loop_v38_mars_winner_strong.json" \
    "$OPEN_ROOT/open_loop_v38_mars_pair_only.json" \
    "$OPEN_ROOT/open_loop_v38_mars_actionrank_control.json" \
    "${baseline_args[@]}" \
...
run_train() {
  ...
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --log-file "$LOG_ROOT/v38_mars_train.jsonl" \
    > "$LOG_ROOT/v38_mars_train.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V38_CKPT" --device cuda \
...
run_closed_loop_group() {
  local limit="$1"
  [[ -f "$V38_CKPT" ]] || { echo "Missing checkpoint: $V38_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_MAIN:-0}" v38_mars_main "$V38_MAIN_CFG" "$limit" &
  run_closed_loop_one "${GPU_CL_SAFETY:-0}" v38_mars_safety bdse/configs/v38_bdse_mars_safety_fallback_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_BBR:-1}" v38_mars_bbr bdse/configs/v38_bdse_mars_bbr_scur_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v38_mars_main_${limit}" "$CL_ROOT/v38_mars_safety_${limit}" "$CL_ROOT/v38_mars_bbr_${limit}" \
    --csv "$CL_ROOT/v38_${limit}_compare.csv"
}
