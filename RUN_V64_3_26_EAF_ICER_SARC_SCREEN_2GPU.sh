...
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_26_eaf_icer_sarc_screen_2gpu_v1}"
...
export DESIGN_EXCLUDE_TOKENS="${DESIGN_EXCLUDE_TOKENS:-bdse/configs/v64_3_26_design_exclude_v64_3_25_screen_tokens.txt}"
export FRESH_HASH_SEED="${FRESH_HASH_SEED:-v64.3.26-eaf-icer-sarc-double-fresh-v1}"
...
TRAIN_TOKENS="$OUT_ROOT/provenance/v64_3_26_train_tokens.txt"
FIT_DIR="$OUT_ROOT/configs"
mkdir -p "$OUT_ROOT/provenance" "$OUT_ROOT/logs" "$FIT_DIR"
TIMING="$OUT_ROOT/provenance/v64_3_26_stage_timing.tsv"; : > "$TIMING"
ALL_START=$(date +%s)
...
[[ -s "$DESIGN_EXCLUDE_TOKENS" ]] || { echo 'STOP: missing V26 design exclusion' >&2; exit 2; }
python - "$DESIGN_EXCLUDE_TOKENS" <<'PY'
import sys
x=[t.strip() for t in open(sys.argv[1]) if t.strip()]
if len(x)!=6700 or len(set(x))!=6700:
    raise SystemExit(f'STOP DATA: V26 design exclusion must be exactly 6700 unique inspected validation tokens, got rows={len(x)} unique={len(set(x))}')
print('PASS V26 frozen 6700-token design exclusion')
PY
...
python -m bdse.tools.check_v64_3_13_eaf_dmvr_screen \
  --train-log "$EAF_TRAIN_LOG" --variant REAUDIT_FOR_V64_3_26 \
...
# V26 needs one new TRAIN instrumentation replay because historical V24/V25
# frontier provenance did not serialize fixed semantic-family attribution
# coordinates.  This is instrumentation only: V20/EAF/selector/frontier are
# frozen, and the new features are diagnostics until the TRAIN fitter constructs
# the two risk-memory arms.  A previously completed V26 frontier may be reused.
stage_start train_semantic_frontier_reuse_or_replay
TRAIN_EDGES=""
TRAIN_COMPLETE_MARKER="$OUT_ROOT/provenance/v64_3_26_train_semantic_frontier_complete.ok"
if [[ -n "${V26_TRAIN_EDGES:-}" && -s "${V26_TRAIN_EDGES}" ]]; then
  TRAIN_EDGES="$V26_TRAIN_EDGES"
elif [[ -s "$OUT_ROOT/provenance/train_v20_semantic_family_frontier_edges.jsonl" && -s "$TRAIN_COMPLETE_MARKER" ]]; then
  TRAIN_EDGES="$OUT_ROOT/provenance/train_v20_semantic_family_frontier_edges.jsonl"
fi
if [[ -n "$TRAIN_EDGES" ]]; then
  printf 'reuse\t%s\n' "$TRAIN_EDGES" > "$OUT_ROOT/provenance/v64_3_26_train_frontier_source.tsv"
  echo "REUSE V26 semantic TRAIN frontier: $TRAIN_EDGES" | tee "$OUT_ROOT/logs/train_frontier_source.out"
else
  TRAIN_EDGES="$OUT_ROOT/provenance/train_v20_semantic_family_frontier_edges.jsonl"
  printf 'replay\t%s\n' "$TRAIN_EDGES" > "$OUT_ROOT/provenance/v64_3_26_train_frontier_source.tsv"
  echo 'Replaying frozen 3000-scene V20/EAF TRAIN frontier once to instrument semantic-family attribution coordinates.' | tee "$OUT_ROOT/logs/train_frontier_source.out"
  CUDA_VISIBLE_DEVICES="$GPU0" python -m bdse.experiments.evaluate_open_loop \
    ...
    --output "$OUT_ROOT/provenance/train_v20_semantic_family_metrics.json" \
    --frontier-edge-output "$TRAIN_EDGES" --disable-dense-diagnostic --device cuda \
    > "$OUT_ROOT/logs/train_semantic_frontier_replay.out" 2>&1
  printf 'complete\n' > "$TRAIN_COMPLETE_MARKER"
fi
[[ -s "$TRAIN_EDGES" ]] || { echo 'STOP TRAIN: missing semantic-family TRAIN frontier' >&2; exit 2; }
stage_end

stage_start train_sarc_fit
set +e
python -m bdse.tools.fit_v64_3_26_eaf_icer_sarc \
  --train-frontier-edges "$TRAIN_EDGES" \
  --base-v20-dual-config "$V20_CONFIG" \
  --output-dir "$FIT_DIR" \
  --output-train-token-file "$TRAIN_TOKENS" \
  --output-report "$OUT_ROOT/provenance/v64_3_26_train_sarc_fit.json" \
  2>&1 | tee "$OUT_ROOT/logs/v64_3_26_sarc_fit.out"
FIT_STATUS=${PIPESTATUS[0]}
set -e
stage_end
if [[ "$FIT_STATUS" -ne 0 ]]; then
  echo "STOP TRAIN SARC (status=$FIT_STATUS). Audit report retained at $OUT_ROOT/provenance/v64_3_26_train_sarc_fit.json" >&2
  exit "$FIT_STATUS"
fi
for spec in aggregate-downside semantic-family-downside; do
  f=${spec//-/_}
  python -m bdse.tools.check_v64_3_26_eaf_icer_sarc_contract \
    --config "$FIT_DIR/v64_3_26_${f}.yaml" --expect "$spec" \
...
python -m bdse.tools.select_fresh_preprocessed_tokens \
  ...
  --audit-output "$OUT_ROOT/provenance/v64_3_26_fresh_1000_audit.json" \
...
print('PASS V26 train/design/fresh identity isolation')
PY
stage_end
...
AGD="$FIT_DIR/v64_3_26_aggregate_downside.yaml"
SFD="$FIT_DIR/v64_3_26_semantic_family_downside.yaml"
# Four arms per independent block.  V25 aggregate DRC is the sole mechanism
# control; V26 changes only the regret-risk representation.
for sp in A B; do
  ...
  run_eval "$GPU1" "$SFD" "$sp" semantic_family_downside & p1=$!
  s=0; wait "$p0" || s=1; wait "$p1" || s=1; [[ $s -eq 0 ]] || exit 2
  stage_end
done
...
root=pathlib.Path(sys.argv[1]); tags=['raw','v20','aggregate_downside','semantic_family_downside']
...
print('PASS V26 paired identity: manifest sets exact and 8/8 arms share within-split row order')
PY
stage_end

stage_start screen
for sp in A B; do
  python -m bdse.tools.check_v64_3_26_eaf_icer_sarc_split --split-name "$sp" \
    ...
    --semantic-family-downside-metrics "$OUT_ROOT/provenance/${sp}_semantic_family_downside_metrics.json" --semantic-family-downside-rows "$OUT_ROOT/provenance/${sp}_semantic_family_downside_rows.jsonl" --semantic-family-downside-edges "$OUT_ROOT/provenance/${sp}_semantic_family_downside_edges.jsonl" \
    --output "$OUT_ROOT/provenance/v64_3_26_split_${sp}_screen.json" | tee "$OUT_ROOT/logs/v64_3_26_split_${sp}_screen.out"
done
SCREEN="$OUT_ROOT/provenance/v64_3_26_eaf_icer_sarc_double_fresh_screen.json"
python -m bdse.tools.check_v64_3_26_eaf_icer_sarc_screen \
  --split-a-report "$OUT_ROOT/provenance/v64_3_26_split_A_screen.json" \
  --split-b-report "$OUT_ROOT/provenance/v64_3_26_split_B_screen.json" \
  --train-fit-report "$OUT_ROOT/provenance/v64_3_26_train_sarc_fit.json" \
  --output "$SCREEN" | tee "$OUT_ROOT/logs/v64_3_26_double_fresh_screen.out"
stage_end
printf 'TOTAL\t%s\n' "$(( $(date +%s)-ALL_START ))" >> "$TIMING"
python - "$SCREEN" <<'PY'
import json,sys
r=json.load(open(sys.argv[1])); print('DOUBLE_FRESH_PASS=',r.get('both_independent_blocks_pass')); print('SEMANTIC_TAIL_INCREMENTAL_BOTH=',r.get('semantic_family_tail_incremental_both')); print('NEXT_ACTION=',r.get('next_action'))
