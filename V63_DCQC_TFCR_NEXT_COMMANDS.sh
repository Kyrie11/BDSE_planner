...
# v63.0 checkpoint-independent pipeline.  Resolve every relative path from the
# checked-in script directory so a stale working directory cannot silently run a
# different config or helper script.
PIPELINE_VERSION="v63.1-deployment-consistent-query-contract-teacher-flip-criticality-ranking"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
echo "[v63] pipeline_version=$PIPELINE_VERSION script=$SCRIPT_DIR/$(basename "$0")"
...
export OUT_ROOT="${OUT_ROOT:-outputs_v63_dcqc_tfcr_fast_2gpu_v1}"
...
# alias consumed by run_v63_dcqc_tfcr.sh.
...
export V63_INIT_MODE="${V63_INIT_MODE:-warm_start}"
...
export FOUNDATION_CONTROL_CONFIG="${FOUNDATION_CONTROL_CONFIG:-bdse/configs/v63_dcqc_tfcr_anchor_control_cl.yaml}"
export LOCAL_CONTROL_CONFIG="${LOCAL_CONTROL_CONFIG:-bdse/configs/v63_dcqc_tfcr_local_control_cl.yaml}"
export TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v63_dcqc_tfcr_train_2gpu.yaml}"
export EVAL_CONFIG="${EVAL_CONFIG:-bdse/configs/v63_dcqc_tfcr_cl.yaml}"
...
# run_v63_dcqc_tfcr.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v63] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi

PIPELINE_LOCK_DIR="$OUT_ROOT/.v63_pipeline.lock"
if ! mkdir "$PIPELINE_LOCK_DIR" 2>/dev/null; then
  owner="$(cat "$PIPELINE_LOCK_DIR/pid" 2>/dev/null || echo unknown)"
  echo "A v63 pipeline is already using OUT_ROOT=$OUT_ROOT (pid=$owner)" >&2
  exit 3
fi
...
training_complete() {
  ...
  [[ -s "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.train_log.jsonl"      && -s "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.pt"      && -s "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.best.pt"      && "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.pt" -nt "$TRAIN_CONFIG" ]]
}
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v63] stage 0 already complete: reuse group-disjoint split"
else
...
# They are algorithm-specific initializations and would confound the v63 paper
...
resolve_foundation_checkpoint() {
  ...
  if [[ -n "$FOUNDATION_CKPT" && ! -s "$FOUNDATION_CKPT" ]]; then
    echo "[v63] ignore missing inherited foundation path: $FOUNDATION_CKPT"
    FOUNDATION_CKPT=""
  fi
  ...
  if [[ "$FOUNDATION_POLICY" != "rebuild" && "$RECOVER_SAFE_FOUNDATION_COPIES" == "1" ]]; then
    ...
    if [[ "$ALLOW_ALGORITHM_CHECKPOINT_INIT" == "1" ]]; then
      ...
      echo "[v63] WARNING: algorithm-specific checkpoint recovery enabled; use only as a transfer ablation" >&2
    fi
    ...
    if [[ -n "$selected" && -s "$selected" ]]; then
      ...
      echo "[v63] recovered foundation checkpoint=$FOUNDATION_CKPT source=$FOUNDATION_SOURCE"
      return 0
    fi
  else
...
if [[ "$V63_INIT_MODE" == "warm_start" ]]; then
  resolve_foundation_checkpoint
  ...
  echo "[v63] V63_INIT_MODE=scratch is an ablation, not the paper main run."
  [[ -n "$CONTROL_CKPT" && -s "$CONTROL_CKPT" ]] || {
    echo "Scratch v63 still needs an independently trained matched CONTROL_CKPT for a paired gate" >&2
    exit 2
  }
...
echo "[v63] foundation_source=$FOUNDATION_SOURCE"
echo "[v63] foundation_checkpoint=$FOUNDATION_CKPT"
echo "[v63] control_checkpoint=$CONTROL_CKPT"
...
#      uninterpretable, so the algorithm stage is blocked before any v63 update.
# ---------------------------------------------------------------------------
...
if is_fresh "$FOUNDATION_QUALITY_JSON" "$FOUNDATION_CKPT" "$FOUNDATION_CONTROL_CONFIG" \
  ...
  echo "[v63] foundation quality replay already complete"
else
...
ANCHOR_GATE_REPORT="$OUT_ROOT/provenance/v63_anchor_gate.json"
python -m bdse.tools.check_v53_anchor_quality "$FOUNDATION_QUALITY_JSON" \
  ...
  2>&1 | tee "$OUT_ROOT/logs/v63_anchor_quality_gate.out"

# ---------------------------------------------------------------------------
# 1. Main v63 training. Base/local encoders are frozen by config; only the
#    reset residual/uncertainty and proposal-family heads are trainable.
# ---------------------------------------------------------------------------
if training_complete; then
  echo "[v63] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS="$GPUS" \
  V30_CKPT_IN="$V30_CKPT_IN" \
  INIT_MODE="$V63_INIT_MODE" \
  ...
  bash run_v63_dcqc_tfcr.sh
fi

BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v63_dcqc_tfcr.best.pt"
[[ -s "$BEST_CHECKPOINT" ]] || { echo "Missing v63 best checkpoint: $BEST_CHECKPOINT" >&2; exit 2; }
...
#    not calibrate the direct residual uncertainty used by the flip guard.  V63
...
DUAL_CAL_JSON="$OUT_ROOT/calibration/v63_dual_certificate.json"
...
prepare_calibration_shards() {
  if [[ "$PIPELINE_FORCE" != "1" \
    ...
    echo "[v63] reuse calibration shard manifests"
    return 0
  fi
...
launch_calibration_shard() {
  ...
  if calibration_shard_fresh "$raw" "$manifest"; then
    echo "[v63] calibration shard gpu${sid} already complete"
    return 1
  fi
  rm -f "$raw" "$raw.failed"
  echo "[v63] launch calibration shard gpu${sid} on CUDA_VISIBLE_DEVICES=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -m bdse.tools.calibrate_v61_dual_certificates \
    --raw-output "$raw" \
...
wait_calibration_shard() {
  ...
  if wait "$pid"; then
    ...
    echo "[v63] calibration shard gpu${sid} complete"
    return 0
  fi
  status=$?
  printf 'exit_status=%s\n' "$status" > "$OUT_ROOT/calibration/raw/gpu${sid}.npz.failed"
  echo "[v63] calibration shard gpu${sid} failed with status=$status; tail follows" >&2
  tail -120 "$log" >&2 || true
  return "$status"
}
...
if ! is_fresh "$DUAL_CAL_JSON" "$BEST_CHECKPOINT" "$EVAL_CONFIG" "$CALIBRATION_PROVENANCE"; then
  ...
  [[ -n "$gpu0" && -n "$gpu1" && "$gpu0" != "$gpu1" ]] || {
    echo "V63 calibration requires two distinct GPU ids in GPUS, got GPUS=$GPUS" >&2
    exit 2
  }
  pid_cal0=""; pid_cal1=""
  ...
  [[ "$cal_failed" == "0" ]] || {
    echo "[v63] calibration incomplete. Re-run this script with PIPELINE_FORCE=0; completed shards will be reused." >&2
    exit 4
  }
  ...
  [[ -s "$tmp_cal" ]] || { echo "[v63] merged calibration JSON missing" >&2; exit 4; }
  mv -f "$tmp_cal" "$DUAL_CAL_JSON"
else
  echo "[v63] reuse dual-certificate calibration"
fi

CAND_CAL_CFG="$OUT_ROOT/calibration/v63_dcqc_tfcr_candidate_calibrated.yaml"
LOCAL_CAL_CFG="$OUT_ROOT/calibration/v63_dcqc_tfcr_local_control_calibrated.yaml"
FOUND_CAL_CFG="$OUT_ROOT/calibration/v63_dcqc_tfcr_foundation_control_calibrated.yaml"
python -m bdse.tools.apply_v61_dual_calibration --config "$EVAL_CONFIG" --calibration-json "$DUAL_CAL_JSON" --output "$CAND_CAL_CFG"
...
# ---------------------------------------------------------------------------
PROTOCOL_GATE_MARKER="$OUT_ROOT/open_loop/.v63_protocol_gate_passed"
COMP_GATE_MARKER="$OUT_ROOT/open_loop/.v63_competitive_gate_passed"
GATE_REPORT="$OUT_ROOT/open_loop/v63_dcqc_tfcr_gate_report.json"
latency_gate_args=()
if [[ "$ENFORCE_LATENCY_BEFORE_CL" == "1" ]]; then latency_gate_args+=(--enforce-latency); fi
python -m bdse.tools.check_v63_dcqc_tfcr_gate \
  ...
  --train-log "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.train_log.jsonl" \
  ...
  --report-json "$GATE_REPORT" --latency-target-ms "${V63_LATENCY_TARGET_MS:-500}" \
  "${latency_gate_args[@]}" 2>&1 | tee "$OUT_ROOT/logs/v63_dcqc_tfcr_gate.out"
...
[[ "$PROTOCOL_PASS" == "1" ]] || { echo "[v63] protocol gate failed; closed loop is invalid" >&2; exit 3; }
...
if [[ "$RUN_CLOSED_LOOP_AFTER_GATE" == "1" && ( "$MINIMUM_PASS" == "1" || "$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL" == "1" ) ]]; then
  ...
  if [[ "$MINIMUM_PASS" != "1" ]]; then
    echo "[v63] minimum gate failed; running paired CL20 as diagnostic evidence only"
  ...
  V63_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$CAND_CAL_CFG" \
  bash run_v63_dcqc_tfcr.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$LOCAL_CONTROL_ROOT" RUN_MODE=cl20 \
  V63_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$LOCAL_CAL_CFG" \
  bash run_v63_dcqc_tfcr.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$FOUNDATION_CONTROL_ROOT" RUN_MODE=cl20 \
  V63_CKPT="$FOUNDATION_CKPT" EVAL_CONFIG="$FOUND_CAL_CFG" \
  bash run_v63_dcqc_tfcr.sh
  ...
  python - \
    "$OUT_ROOT/closed_loop/v63_dcqc_tfcr_20/closed_loop_combined_summary.json" \
    "$LOCAL_CONTROL_ROOT/closed_loop/v63_dcqc_tfcr_20/closed_loop_combined_summary.json" \
    "$FOUNDATION_CONTROL_ROOT/closed_loop/v63_dcqc_tfcr_20/closed_loop_combined_summary.json" \
    "$OUT_ROOT/closed_loop/v63_three_way_cl20_protocol.json" <<'PY_CL_PROTOCOL'
import json, sys
from pathlib import Path
...
PY_CL_PROTOCOL
  ...
  if [[ "$RUN_CL100_AFTER_CL20" == "1" && "$COMPETITIVE_PASS" == "1" ]]; then
    ...
    echo "[v63] CL100 requested but competitive gate failed; run paired CL20 first and inspect $GATE_REPORT"
  fi
else
  echo "[v63] closed-loop skipped (RUN_CLOSED_LOOP_AFTER_GATE=$RUN_CLOSED_LOOP_AFTER_GATE, minimum_pass=$MINIMUM_PASS, diagnostic=$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL)"
fi
...
#   --config "$CAND_CAL_CFG" \
#   --checkpoint "$OUT_ROOT/train/bdse_v63_dcqc_tfcr.best.pt" \
#   --split public_set_test --preprocessed-dir "$BDSE_TEST_CACHE" --device cuda \
#   --output "$OUT_ROOT/open_loop/open_loop_v63_dcqc_tfcr_test.json" \
#   --per-sample-output "$OUT_ROOT/open_loop/open_loop_v63_dcqc_tfcr_test.jsonl"
