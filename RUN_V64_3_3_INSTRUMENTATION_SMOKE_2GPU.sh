...
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_3_apwcca_instrumentation_smoke_2gpu_v1}"
export GPUS="${GPUS:-0,1}"
export NPROC_PER_NODE=2
export TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v64_3_3_cc_aocc_apwcca_daepc_smoke_2gpu.yaml}"
export EVAL_CONFIG="${EVAL_CONFIG:-bdse/configs/v64_3_cc_aocc_apwcca_cl.yaml}"
export MAX_TRAIN_SCENARIOS="${MAX_TRAIN_SCENARIOS:-2000}"
export VAL_SCENARIOS="${VAL_SCENARIOS:-200}"
export VAL_EVERY_N_EPOCHS=1
export VAL_BEFORE_TRAINING=1
export BEST_MIN_EPOCH=0
...
files=[
  ...
  'RUN_V64_3_3_INSTRUMENTATION_SMOKE_2GPU.sh',
]
out={}
for name in files:
...
python -m bdse.tools.check_v64_3_3_acquisition_screen \
  ...
  --variant AP-WCCA+ACRA-instrumentation-smoke
