...
# Run this script from the v49 code root, or copy commands selectively.
# Mandatory paths.
...
export BDSE_SPLIT_CACHE="${BDSE_SPLIT_CACHE:-/data0/senzeyu2/dataset/nuplan/data/cache/bdse_val_v49_split}"
...
export OUT_ROOT="${OUT_ROOT:-outputs_v49_dbap_exact_2gpu_v1}"
...
# run_v49_dbap.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v49] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi

PIPELINE_LOCK_DIR="$OUT_ROOT/.v49_pipeline.lock"
if ! mkdir "$PIPELINE_LOCK_DIR" 2>/dev/null; then
  owner="$(cat "$PIPELINE_LOCK_DIR/pid" 2>/dev/null || echo unknown)"
  echo "A v49 pipeline is already using OUT_ROOT=$OUT_ROOT (pid=$owner)" >&2
  exit 3
fi
...
training_complete() {
  ...
  python - \
    ...
    "$OUT_ROOT/train/bdse_v49_dbap.train_log.jsonl" \
    "$OUT_ROOT/train/bdse_v49_dbap.pt" \
    "$OUT_ROOT/train/bdse_v49_dbap.best.pt" <<'PY'
import json
import sys
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v49] stage 0 already complete: reuse group-disjoint split"
else
...
# ---------------------------------------------------------------------------
# 1. Main v49 training. Use val_tune only for checkpoint selection.
#    Main paper run starts from the frozen v30 checkpoint, not v46.
# ---------------------------------------------------------------------------
if training_complete; then
  echo "[v49] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS=0,1 \
...
CALIBRATION_JSON="$OUT_ROOT/calibration/v49_adverse_calibration.json"
CALIBRATED_CONFIG="$OUT_ROOT/calibration/v49_bdse_dbap_cl_calibrated.yaml"
BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v49_dbap.best.pt"
if is_fresh \
  "$CALIBRATION_JSON" \
  "$BEST_CHECKPOINT" \
  "$BDSE_SPLIT_CACHE/calibration_split_provenance.json" \
  bdse/configs/v49_bdse_dbap_cl.yaml; then
  echo "[v49] stage 2a already complete: reuse adverse calibration"
else
  python -m bdse.tools.calibrate_v48_adverse_bounds \
    --config bdse/configs/v49_bdse_dbap_cl.yaml \
...
if is_fresh "$CALIBRATED_CONFIG" "$CALIBRATION_JSON" bdse/configs/v49_bdse_dbap_cl.yaml; then
  echo "[v49] stage 2b already complete: reuse calibrated config"
else
  python -m bdse.tools.apply_v48_calibration \
    --config-in bdse/configs/v49_bdse_dbap_cl.yaml \
    --calibration-json "$CALIBRATION_JSON" \
    --config-out "$CALIBRATED_CONFIG"
fi

# ---------------------------------------------------------------------------
# 3. Replay calibrated v49 on the same deterministic val_tune 1000 scenes.
# ---------------------------------------------------------------------------
OPEN_LOOP_JSON="$OUT_ROOT/open_loop/open_loop_v49_dbap.json"
OPEN_LOOP_JSONL="$OUT_ROOT/open_loop/open_loop_v49_dbap.jsonl"
if is_fresh "$OPEN_LOOP_JSON" "$BEST_CHECKPOINT" "$CALIBRATED_CONFIG" \
  ...
  echo "[v49] stage 3 already complete: reuse calibrated open-loop results"
else
  GPUS=0,1 \
  OUT_ROOT="$OUT_ROOT" \
  RUN_MODE=open_loop \
  V49_CKPT="$BEST_CHECKPOINT" \
...
if is_fresh "$CONTROL_JSON" "$CONTROL_CONFIG" "$CONTROL_CKPT" \
  ...
  echo "[v49] stage 4 already complete: reuse frozen-control results"
else
...
# ---------------------------------------------------------------------------
GATE_MARKER="$OUT_ROOT/open_loop/.v49_dbap_gate_passed"
if is_fresh \
  "$GATE_MARKER" \
  "$OPEN_LOOP_JSON" "$OPEN_LOOP_JSONL" \
  "$CONTROL_JSON" "$CONTROL_JSONL" \
  "$OUT_ROOT/train/bdse_v49_dbap.train_log.jsonl"; then
  echo "[v49] stage 5 already complete: paired gate previously passed"
else
  python -m bdse.tools.check_v48_dbce_gate \
    ...
    --train-log "$OUT_ROOT/train/bdse_v49_dbap.train_log.jsonl" \
    ...
    2>&1 | tee "$OUT_ROOT/logs/v49_dbap_gate.out"
  printf 'passed\n' >"$GATE_MARKER"
fi

# ---------------------------------------------------------------------------
# 6. Only after PASS: paired CL20.  V47's script contained only a commented
#    example, so even a future PASS would not have started simulation.  V49
...
if [[ "$RUN_CLOSED_LOOP_AFTER_GATE" == "1" ]]; then
  ...
  V49_CKPT="$OUT_ROOT/train/bdse_v49_dbap.best.pt" \
  EVAL_CONFIG="$OUT_ROOT/calibration/v49_bdse_dbap_cl_calibrated.yaml" \
  bash run_v49_dbap.sh
  ...
  echo "[v49] gate passed; closed-loop skipped because RUN_CLOSED_LOOP_AFTER_GATE=0"
fi
...
#
# CUDA_VISIBLE_DEVICES=0 python -m bdse.experiments.evaluate_open_loop \
#   --config "$OUT_ROOT/calibration/v49_bdse_dbap_cl_calibrated.yaml" \
#   --checkpoint "$OUT_ROOT/train/bdse_v49_dbap.best.pt" \
#   --split test --preprocessed-dir "$BDSE_TEST_CACHE" --device cuda \
#   --output "$OUT_ROOT/open_loop/open_loop_v49_dbap_test.json" \
#   --per-sample-output "$OUT_ROOT/open_loop/open_loop_v49_dbap_test.jsonl"
