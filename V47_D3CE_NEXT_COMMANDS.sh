...
# run_v47_d3ce.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v47] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi
...
training_complete() {
  ...
  python - \
    ...
    "$OUT_ROOT/train/bdse_v47_d3ce.train_log.jsonl" \
    "$OUT_ROOT/train/bdse_v47_d3ce.pt" \
    "$OUT_ROOT/train/bdse_v47_d3ce.best.pt" <<'PY'
import json
import sys
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v47] stage 0 already complete: reuse group-disjoint split"
else
...
if training_complete; then
  echo "[v47] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS=0,1 \
...
CALIBRATION_JSON="$OUT_ROOT/calibration/v47_adverse_calibration.json"
CALIBRATED_CONFIG="$OUT_ROOT/calibration/v47_bdse_d3ce_cl_calibrated.yaml"
BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v47_d3ce.best.pt"
if is_fresh \
  "$CALIBRATION_JSON" \
  "$BEST_CHECKPOINT" \
  "$BDSE_SPLIT_CACHE/calibration_split_provenance.json" \
  bdse/configs/v47_bdse_d3ce_cl.yaml; then
  echo "[v47] stage 2a already complete: reuse adverse calibration"
else
...
if is_fresh "$CALIBRATED_CONFIG" "$CALIBRATION_JSON" bdse/configs/v47_bdse_d3ce_cl.yaml; then
  echo "[v47] stage 2b already complete: reuse calibrated config"
else
...
# ---------------------------------------------------------------------------
OPEN_LOOP_JSON="$OUT_ROOT/open_loop/open_loop_v47_d3ce.json"
OPEN_LOOP_JSONL="$OUT_ROOT/open_loop/open_loop_v47_d3ce.jsonl"
if is_fresh "$OPEN_LOOP_JSON" "$BEST_CHECKPOINT" "$CALIBRATED_CONFIG" \
  ...
  echo "[v47] stage 3 already complete: reuse calibrated open-loop results"
else
  GPUS=0,1 \
  OUT_ROOT="$OUT_ROOT" \
  RUN_MODE=open_loop \
  V47_CKPT="$BEST_CHECKPOINT" \
...
if is_fresh "$CONTROL_JSON" "$CONTROL_CONFIG" "$CONTROL_CKPT" \
  ...
  echo "[v47] stage 4 already complete: reuse frozen-control results"
else
...
# ---------------------------------------------------------------------------
GATE_MARKER="$OUT_ROOT/open_loop/.v47_d3ce_gate_passed"
if is_fresh \
  "$GATE_MARKER" \
  "$OPEN_LOOP_JSON" "$OPEN_LOOP_JSONL" \
  "$CONTROL_JSON" "$CONTROL_JSONL" \
  "$OUT_ROOT/train/bdse_v47_d3ce.train_log.jsonl"; then
  echo "[v47] stage 5 already complete: paired gate previously passed"
else
  python -m bdse.tools.check_v47_d3ce_gate \
    ...
    2>&1 | tee "$OUT_ROOT/logs/v47_d3ce_gate.out"
  printf 'passed\n' >"$GATE_MARKER"
fi
