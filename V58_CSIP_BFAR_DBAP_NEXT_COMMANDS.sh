...
# v58.0 checkpoint-independent pipeline.  Resolve every relative path from the
# checked-in script directory so a stale working directory cannot silently run a
# different config or helper script.
PIPELINE_VERSION="v58.0-certified-set-aligned-integrable-potential-bfar"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
echo "[v58] pipeline_version=$PIPELINE_VERSION script=$SCRIPT_DIR/$(basename "$0")"
...
export OUT_ROOT="${OUT_ROOT:-outputs_v58_csip_bfar_dbap_fast_2gpu_v1}"
...
# alias consumed by run_v58_csip_bfar_dbap.sh.
...
export V58_INIT_MODE="${V58_INIT_MODE:-warm_start}"
...
export FOUNDATION_CONTROL_CONFIG="${FOUNDATION_CONTROL_CONFIG:-bdse/configs/v58_csip_bfar_dbap_anchor_control_cl.yaml}"
export LOCAL_CONTROL_CONFIG="${LOCAL_CONTROL_CONFIG:-bdse/configs/v58_csip_bfar_dbap_local_control_cl.yaml}"
...
# run_v58_csip_bfar_dbap.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v58] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi

PIPELINE_LOCK_DIR="$OUT_ROOT/.v58_pipeline.lock"
if ! mkdir "$PIPELINE_LOCK_DIR" 2>/dev/null; then
  owner="$(cat "$PIPELINE_LOCK_DIR/pid" 2>/dev/null || echo unknown)"
  echo "A v58 pipeline is already using OUT_ROOT=$OUT_ROOT (pid=$owner)" >&2
  exit 3
fi
...
training_complete() {
  ...
  [[ -s "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.train_log.jsonl"      && -s "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.pt"      && -s "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.best.pt"      && "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.pt" -nt bdse/configs/v58_csip_bfar_dbap_train_2gpu.yaml ]]
}
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v58] stage 0 already complete: reuse group-disjoint split"
else
...
# They are algorithm-specific initializations and would confound the v58 paper
...
resolve_foundation_checkpoint() {
  ...
  if [[ -n "$FOUNDATION_CKPT" && ! -s "$FOUNDATION_CKPT" ]]; then
    echo "[v58] ignore missing inherited foundation path: $FOUNDATION_CKPT"
    FOUNDATION_CKPT=""
  fi
  ...
  if [[ "$FOUNDATION_POLICY" != "rebuild" && "$RECOVER_SAFE_FOUNDATION_COPIES" == "1" ]]; then
    ...
    if [[ "$ALLOW_ALGORITHM_CHECKPOINT_INIT" == "1" ]]; then
      ...
      echo "[v58] WARNING: algorithm-specific checkpoint recovery enabled; use only as a transfer ablation" >&2
    fi
    selected="$(python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v58_csip_bfar_dbap_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY"       --print-selected       "${allow_args[@]}")"
    if [[ -n "$selected" && -s "$selected" ]]; then
      ...
      echo "[v58] recovered foundation checkpoint=$FOUNDATION_CKPT source=$FOUNDATION_SOURCE"
      return 0
    fi
  else
    ...
    python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v58_csip_bfar_dbap_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY" >/dev/null || true
  fi
...
if [[ "$V58_INIT_MODE" == "warm_start" ]]; then
  resolve_foundation_checkpoint
  ...
  echo "[v58] V58_INIT_MODE=scratch is an ablation, not the paper main run."
  [[ -n "$CONTROL_CKPT" && -s "$CONTROL_CKPT" ]] || {
    echo "Scratch v58 still needs an independently trained matched CONTROL_CKPT for a paired gate" >&2
    exit 2
  }
...
echo "[v58] foundation_source=$FOUNDATION_SOURCE"
echo "[v58] foundation_checkpoint=$FOUNDATION_CKPT"
echo "[v58] control_checkpoint=$CONTROL_CKPT"
...
#      uninterpretable, so the algorithm stage is blocked before any v58 update.
# ---------------------------------------------------------------------------
...
if is_fresh "$FOUNDATION_QUALITY_JSON" "$FOUNDATION_CKPT" "$FOUNDATION_CONTROL_CONFIG" \
  ...
  echo "[v58] foundation quality replay already complete"
else
...
ANCHOR_GATE_REPORT="$OUT_ROOT/provenance/v58_anchor_gate.json"
python -m bdse.tools.check_v53_anchor_quality "$FOUNDATION_QUALITY_JSON" \
  ...
  2>&1 | tee "$OUT_ROOT/logs/v58_anchor_quality_gate.out"

# ---------------------------------------------------------------------------
# 1. Main v58 training. Base/local encoders are frozen by config; only the
#    reset residual/uncertainty and proposal-family heads are trainable.
# ---------------------------------------------------------------------------
if training_complete; then
  echo "[v58] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS="$GPUS" \
  V30_CKPT_IN="$V30_CKPT_IN" \
  INIT_MODE="$V58_INIT_MODE" \
  TRAIN_CONFIG=bdse/configs/v58_csip_bfar_dbap_train_2gpu.yaml \
...
BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.best.pt"
[[ -s "$BEST_CHECKPOINT" ]] || { echo "Missing v58 best checkpoint: $BEST_CHECKPOINT" >&2; exit 2; }
...
#    not calibrate the direct residual uncertainty used by the flip guard.  V58
...
DUAL_CAL_JSON="$OUT_ROOT/calibration/v58_dual_certificate.json"
...
if ! is_fresh "$DUAL_CAL_JSON" "$BEST_CHECKPOINT" bdse/configs/v58_csip_bfar_dbap_cl.yaml "$CALIBRATION_PROVENANCE"; then
  prepare_calibration_shards
  CUDA_VISIBLE_DEVICES="${GPUS%%,*}" python -m bdse.tools.calibrate_v58_dual_certificates \
    --raw-output "$OUT_ROOT/calibration/raw/gpu0.npz" \
    --config bdse/configs/v58_csip_bfar_dbap_cl.yaml --checkpoint "$BEST_CHECKPOINT" \
    --preprocessed-dir "$CAL_SHARD_ROOT/gpu0" --split val --max-scenarios 5000 \
    --device cuda --alpha 0.05 --beta 1.0 --prior-radius 0.02 \
    > "$OUT_ROOT/logs/calibration_gpu0.out" 2>&1 & pid_cal0=$!
  CUDA_VISIBLE_DEVICES="$(echo "$GPUS" | cut -d, -f2)" python -m bdse.tools.calibrate_v58_dual_certificates \
  ...
  python -m bdse.tools.calibrate_v58_dual_certificates \
    --merge-raw "$OUT_ROOT/calibration/raw/gpu0.npz" "$OUT_ROOT/calibration/raw/gpu1.npz" \
    --alpha 0.05 --beta 1.0 --residual-epsilon-fallback 0.05 \
    --provenance-json "$CALIBRATION_PROVENANCE" --output "$DUAL_CAL_JSON"
else
  echo "[v58] reuse dual-certificate calibration"
fi

CAND_CAL_CFG="$OUT_ROOT/calibration/v58_csip_bfar_dbap_candidate_calibrated.yaml"
LOCAL_CAL_CFG="$OUT_ROOT/calibration/v58_csip_bfar_dbap_local_control_calibrated.yaml"
FOUND_CAL_CFG="$OUT_ROOT/calibration/v58_csip_bfar_dbap_foundation_control_calibrated.yaml"
python -m bdse.tools.apply_v58_dual_calibration --config bdse/configs/v58_csip_bfar_dbap_cl.yaml --calibration-json "$DUAL_CAL_JSON" --output "$CAND_CAL_CFG"
python -m bdse.tools.apply_v58_dual_calibration --config "$LOCAL_CONTROL_CONFIG" --calibration-json "$DUAL_CAL_JSON" --output "$LOCAL_CAL_CFG" --control
python -m bdse.tools.apply_v58_dual_calibration --config "$FOUNDATION_CONTROL_CONFIG" --calibration-json "$DUAL_CAL_JSON" --output "$FOUND_CAL_CFG" --control
...
# ---------------------------------------------------------------------------
PROTOCOL_GATE_MARKER="$OUT_ROOT/open_loop/.v58_protocol_gate_passed"
COMP_GATE_MARKER="$OUT_ROOT/open_loop/.v58_competitive_gate_passed"
GATE_REPORT="$OUT_ROOT/open_loop/v58_csip_bfar_gate_report.json"
latency_gate_args=()
if [[ "$ENFORCE_LATENCY_BEFORE_CL" == "1" ]]; then latency_gate_args+=(--enforce-latency); fi
python -m bdse.tools.check_v58_csip_bfar_dbap_gate \
  ...
  --train-log "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.train_log.jsonl" \
  --train-config bdse/configs/v58_csip_bfar_dbap_train_2gpu.yaml \
  --candidate-config "$CAND_CAL_CFG" \
  --local-control-config "$LOCAL_CAL_CFG" \
  --foundation-control-config "$FOUND_CAL_CFG" \
  --report-json "$GATE_REPORT" --latency-target-ms "${V58_LATENCY_TARGET_MS:-500}" \
  "${latency_gate_args[@]}" 2>&1 | tee "$OUT_ROOT/logs/v58_csip_bfar_dbap_gate.out"
...
[[ "$PROTOCOL_PASS" == "1" ]] || { echo "[v58] protocol gate failed; closed loop is invalid" >&2; exit 3; }
...
if [[ "$RUN_CLOSED_LOOP_AFTER_GATE" == "1" && ( "$MINIMUM_PASS" == "1" || "$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL" == "1" ) ]]; then
  ...
  if [[ "$MINIMUM_PASS" != "1" ]]; then
    echo "[v58] minimum gate failed; running paired CL20 as diagnostic evidence only"
  ...
  V58_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$CAND_CAL_CFG" \
  bash run_v58_csip_bfar_dbap.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$LOCAL_CONTROL_ROOT" RUN_MODE=cl20 \
  V58_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$LOCAL_CAL_CFG" \
  bash run_v58_csip_bfar_dbap.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$FOUNDATION_CONTROL_ROOT" RUN_MODE=cl20 \
  V58_CKPT="$FOUNDATION_CKPT" EVAL_CONFIG="$FOUND_CAL_CFG" \
  bash run_v58_csip_bfar_dbap.sh
  ...
  python - \
    "$OUT_ROOT/closed_loop/v58_csip_bfar_dbap_20/closed_loop_combined_summary.json" \
    "$LOCAL_CONTROL_ROOT/closed_loop/v58_csip_bfar_dbap_20/closed_loop_combined_summary.json" \
    "$FOUNDATION_CONTROL_ROOT/closed_loop/v58_csip_bfar_dbap_20/closed_loop_combined_summary.json" \
    "$OUT_ROOT/closed_loop/v58_three_way_cl20_protocol.json" <<'PY_CL_PROTOCOL'
import json, sys
from pathlib import Path
...
PY_CL_PROTOCOL
  ...
  if [[ "$RUN_CL100_AFTER_CL20" == "1" && "$COMPETITIVE_PASS" == "1" ]]; then
    ...
    echo "[v58] CL100 requested but competitive gate failed; run paired CL20 first and inspect $GATE_REPORT"
  fi
else
  echo "[v58] closed-loop skipped (RUN_CLOSED_LOOP_AFTER_GATE=$RUN_CLOSED_LOOP_AFTER_GATE, minimum_pass=$MINIMUM_PASS, diagnostic=$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL)"
fi
...
#   --config "$CAND_CAL_CFG" \
#   --checkpoint "$OUT_ROOT/train/bdse_v58_csip_bfar_dbap.best.pt" \
#   --split public_set_test --preprocessed-dir "$BDSE_TEST_CACHE" --device cuda \
#   --output "$OUT_ROOT/open_loop/open_loop_v58_csip_bfar_dbap_test.json" \
#   --per-sample-output "$OUT_ROOT/open_loop/open_loop_v58_csip_bfar_dbap_test.jsonl"
