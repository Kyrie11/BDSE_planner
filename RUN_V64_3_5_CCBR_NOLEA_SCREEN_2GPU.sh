...
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_5_ccbr_nolea_activation_screen_2gpu_v1}"
export GPUS="${GPUS:-0,1}"
export NPROC_PER_NODE=2
export TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v64_3_5_cc_aocc_ccbr_nolea_daepc_screen_2gpu.yaml}"
...
python -m bdse.tools.validate_v64_pipeline_config \
  ...
  --expected-family v64.3.5 \
  --output "$OUT_ROOT/provenance/config_contract.json"
python - "$OUT_ROOT/provenance/screen_code_sha256.json" <<'PY_SHA'
import hashlib,json,sys
from pathlib import Path
files=['bdse/model/bdse_model.py','bdse/model/losses.py','bdse/experiments/evaluate_open_loop.py','bdse/experiments/train.py','RUN_V64_3_5_CCBR_NOLEA_SCREEN_2GPU.sh']
out={name:hashlib.sha256(Path(name).read_bytes()).hexdigest() for name in files}
...
python -m bdse.tools.check_v64_3_3_acquisition_screen \
  ...
  --variant CCBR-noLEA+ACRA-full-support
