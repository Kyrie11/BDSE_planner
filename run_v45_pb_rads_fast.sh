...
# Allow either:
#   GPUS=0,1 RUN_MODE=train_open_loop bash run_v45_pb_rads.sh
# or:
#   bash run_v45_pb_rads.sh GPUS=0,1 RUN_MODE=train_open_loop
for kv in "$@"; do
  [[ "$kv" == *=* ]] && export "$kv"
done
...
OUT_ROOT="${OUT_ROOT:-outputs_v45_pb_rads_fast_2gpu}"
...
EVAL_CONFIG="${EVAL_CONFIG:-bdse/configs/v45_bdse_pb_rads_fast_cl.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v45_bdse_pb_rads_train_fast_2gpu.yaml}"
...
EXACT_DISTILL_SCENES_PER_RANK="${EXACT_DISTILL_SCENES_PER_RANK:-1}"
EXACT_DISTILL_EVERY_N_STEPS="${EXACT_DISTILL_EVERY_N_STEPS:-4}"
...
if [[ "$DETACH" == "1" && "${BDSE_DETACHED_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v45] detached session started pid=$child_pid log=$launcher_log"
  exit 0
fi
CKPT="$OUT_ROOT/train/bdse_v45_pb_rads_fast.pt"
BEST_CKPT="$OUT_ROOT/train/bdse_v45_pb_rads_fast.best.pt"
LATEST_CKPT="$OUT_ROOT/train/bdse_v45_pb_rads_fast.latest.pt"
...
train_2gpu() {
  ...
  if [[ "$AUTO_RESUME" == "1" && -f "$LATEST_CKPT" ]]; then
    ...
    echo "[v45] auto-resume checkpoint=$LATEST_CKPT"
  else
    check_checkpoint "$V30_CKPT_IN"
    checkpoint_args+=(--warm-start-from "$V30_CKPT_IN")
    echo "[v45] warm-start checkpoint=$V30_CKPT_IN"
  fi
  echo "[v45] DDP training on physical GPUs $GPU0,$GPU1"
  echo "[v45] batch_per_gpu=$BATCH_SIZE_PER_GPU global_batch=$effective_global_batch workers_per_gpu=$NUM_WORKERS_PER_GPU"
  echo "[v45] train_config=$TRAIN_CONFIG val_scenarios=$VAL_SCENARIOS val_every=$VAL_EVERY_N_EPOCHS dense_val=$VAL_DENSE_DIAGNOSTIC"
  echo "[v45] auto_resume=$AUTO_RESUME save_every_n_steps=$SAVE_EVERY_N_STEPS"
  echo "[v45-fast] gpu_surrogate=on exact_distill_scenes_per_rank=$EXACT_DISTILL_SCENES_PER_RANK exact_distill_every_n_steps=$EXACT_DISTILL_EVERY_N_STEPS"
  ...
  torchrun \
    ...
    -m bdse.experiments.train \
      ...
      --log-file "$OUT_ROOT/train/bdse_v45_pb_rads_fast.train_log.jsonl" \
      --output "$CKPT" \
...
run_open_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_open_loop \
    ...
    --device cuda \
    --output "$OUT_ROOT/open_loop/open_loop_v45_pb_rads.gpu${shard_id}.json" \
    --per-sample-output "$OUT_ROOT/open_loop/open_loop_v45_pb_rads.gpu${shard_id}.jsonl" \
...
summary_paths = [root / f"open_loop_v45_pb_rads.gpu{i}.json" for i in range(2)]
row_paths = [root / f"open_loop_v45_pb_rads.gpu{i}.jsonl" for i in range(2)]
...
(root / "open_loop_v45_pb_rads.json").write_text(json.dumps(out, indent=2, sort_keys=True), encoding="utf-8")
with (root / "open_loop_v45_pb_rads.jsonl").open("w", encoding="utf-8") as f:
    for row in rows:
...
run_closed_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_closed_loop \
    ...
    --experiment-uid "v45_pb_rads_${limit}_gpu${shard_id}" \
...
closed_loop_2gpu() {
  ...
  local run_root="$OUT_ROOT/closed_loop/v45_pb_rads_${limit}"
  local token_root="$run_root/scenario_token_shards"
  rm -rf "$run_root"
  mkdir -p "$run_root"
  prepare_closed_loop_token_shards "$limit" "$token_root"

  echo "[v45] Closed-loop $limit scenarios split across physical GPUs $GPU0 and $GPU1"
  echo "[v45] Reuse $token_root/scenario_tokens_all.json for every compared method to keep the CL subset identical."
  local start_time end_time
  start_time=$(date +%s)
