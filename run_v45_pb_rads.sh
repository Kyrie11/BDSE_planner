...
OUT_ROOT="${OUT_ROOT:-outputs_v45_pb_rads_2gpu}"
...
TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v45_bdse_pb_rads_train_exact_2gpu.yaml}"
...
VAL_DENSE_DIAGNOSTIC="${VAL_DENSE_DIAGNOSTIC:-1}"
...
CKPT="$OUT_ROOT/train/bdse_v45_pb_rads.pt"
BEST_CKPT="$OUT_ROOT/train/bdse_v45_pb_rads.best.pt"
LATEST_CKPT="$OUT_ROOT/train/bdse_v45_pb_rads.latest.pt"
...
train_2gpu() {
  ...
  echo "[v45] selector_scenes_per_rank=$SELECTOR_SCENES_PER_RANK selector_every_n_steps=$SELECTOR_EVERY_N_STEPS selector_full_last_n_steps=$SELECTOR_FULL_LAST_N_STEPS"
  ...
  torchrun \
    ...
    -m bdse.experiments.train \
      ...
      --log-file "$OUT_ROOT/train/bdse_v45_pb_rads.train_log.jsonl" \
      --output "$CKPT" \
