...
: "${BTP_BDMU_SCREEN_REPORT:?Set BTP_BDMU_SCREEN_REPORT to the promoted V64.3.11 screen report}"
for p in "$DARM_DBR_CKPT" "$DARM_DBR_FULL_REPORT" "$BTP_BDMU_SCREEN_REPORT"; do
  [[ -s "$p" ]] || { echo "missing required artifact: $p" >&2; exit 2; }
done
python - "$DARM_DBR_FULL_REPORT" "$BTP_BDMU_SCREEN_REPORT" <<'PY'
import json,sys
for name,p in [('DARM+DBR full',sys.argv[1]),('BTP-BDMU screen',sys.argv[2])]:
    r=json.load(open(p))
    if not r.get('full_promotion', False):
        raise SystemExit(f'{name} did not meet promotion criteria; BTP-BDMU full training blocked.')
PY
...
python -m bdse.tools.check_v64_3_11_btp_bdmu_screen \
  --train-log "$OUT_ROOT/train/bdse_v64_saqa_bcc.train_log.jsonl" \
  --output "$OUT_ROOT/provenance/v64_3_11_btp_bdmu_full_audit.json"
echo "[v64.3.11] full report: $OUT_ROOT/provenance/v64_3_11_btp_bdmu_full_audit.json"
