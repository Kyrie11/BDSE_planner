...
# v52.1 checkpoint-independent pipeline.  Resolve every relative path from the
# checked-in script directory so a stale working directory cannot silently run a
# different config or helper script.
PIPELINE_VERSION="v52.1-boundary-focused-anchor-residual-fast"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
echo "[v52] pipeline_version=$PIPELINE_VERSION script=$SCRIPT_DIR/$(basename "$0")"
...
export BDSE_SPLIT_CACHE="${BDSE_SPLIT_CACHE:-/data0/senzeyu2/dataset/nuplan/data/cache/bdse_val_v52_split}"
export BDSE_TEST_CACHE="${BDSE_TEST_CACHE:-/data0/senzeyu2/dataset/nuplan/data/cache/bdse_test_v2}"
export OUT_ROOT="${OUT_ROOT:-outputs_v52_bfar_dbap_fast_2gpu_v1}"
export FOUNDATION_ROOT="${FOUNDATION_ROOT:-$OUT_ROOT/factorized_anchor}"
export FOUNDATION_CONFIG="${FOUNDATION_CONFIG:-bdse/configs/v52_factorized_anchor_fast_2gpu.yaml}"
...
# alias consumed by run_v52_bfar_dbap.sh.
...
export V52_INIT_MODE="${V52_INIT_MODE:-warm_start}"
...
export FOUNDATION_CONTROL_CONFIG="${FOUNDATION_CONTROL_CONFIG:-bdse/configs/v52_bfar_dbap_anchor_control_cl.yaml}"
export LOCAL_CONTROL_CONFIG="${LOCAL_CONTROL_CONFIG:-bdse/configs/v52_bfar_dbap_local_control_cl.yaml}"
...
# run_v52_bfar_dbap.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v52] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi

PIPELINE_LOCK_DIR="$OUT_ROOT/.v52_pipeline.lock"
if ! mkdir "$PIPELINE_LOCK_DIR" 2>/dev/null; then
  owner="$(cat "$PIPELINE_LOCK_DIR/pid" 2>/dev/null || echo unknown)"
  echo "A v52 pipeline is already using OUT_ROOT=$OUT_ROOT (pid=$owner)" >&2
  exit 3
fi
...
training_complete() {
  ...
  [[ -s "$OUT_ROOT/train/bdse_v52_bfar_dbap.train_log.jsonl"      && -s "$OUT_ROOT/train/bdse_v52_bfar_dbap.pt"      && -s "$OUT_ROOT/train/bdse_v52_bfar_dbap.best.pt"      && "$OUT_ROOT/train/bdse_v52_bfar_dbap.pt" -nt bdse/configs/v52_bfar_dbap_train_2gpu.yaml ]]
}
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v52] stage 0 already complete: reuse group-disjoint split"
else
...
#   3) a conservatively verified retained compatible anchor copy from outputs_v40..v52;
...
# They are algorithm-specific initializations and would confound the v52 paper
...
resolve_foundation_checkpoint() {
  local rebuilt_best="$FOUNDATION_ROOT/train/bdse_v52_factorized_anchor.best.pt"
  local selected=""
  ...
  if [[ -n "$FOUNDATION_CKPT" && ! -s "$FOUNDATION_CKPT" ]]; then
    echo "[v52] ignore missing inherited foundation path: $FOUNDATION_CKPT"
    FOUNDATION_CKPT=""
  fi
  ...
  if [[ "$FOUNDATION_POLICY" != "rebuild" && "$RECOVER_SAFE_FOUNDATION_COPIES" == "1" ]]; then
    ...
    if [[ "$ALLOW_ALGORITHM_CHECKPOINT_INIT" == "1" ]]; then
      ...
      echo "[v52] WARNING: algorithm-specific checkpoint recovery enabled; use only as a transfer ablation" >&2
    fi
    selected="$(python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v52_bfar_dbap_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY"       --print-selected       "${allow_args[@]}")"
    if [[ -n "$selected" && -s "$selected" ]]; then
      ...
      echo "[v52] recovered foundation checkpoint=$FOUNDATION_CKPT source=$FOUNDATION_SOURCE"
      return 0
    fi
  else
    ...
    python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v52_bfar_dbap_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY" >/dev/null || true
  fi
...
if [[ "$V52_INIT_MODE" == "warm_start" ]]; then
  resolve_foundation_checkpoint
  ...
  echo "[v52] V52_INIT_MODE=scratch is an ablation, not the paper main run."
  [[ -n "$CONTROL_CKPT" && -s "$CONTROL_CKPT" ]] || {
    echo "Scratch v52 still needs an independently trained matched CONTROL_CKPT for a paired gate" >&2
    exit 2
  }
...
echo "[v52] foundation_source=$FOUNDATION_SOURCE"
echo "[v52] foundation_checkpoint=$FOUNDATION_CKPT"
echo "[v52] control_checkpoint=$CONTROL_CKPT"
...
#      uninterpretable, so the algorithm stage is blocked before any v52 update.
# ---------------------------------------------------------------------------
...
if is_fresh "$FOUNDATION_QUALITY_JSON" "$FOUNDATION_CKPT" "$FOUNDATION_CONTROL_CONFIG" \
  ...
  echo "[v52] foundation quality replay already complete"
else
...
python -m bdse.tools.check_v52_anchor_quality "$FOUNDATION_QUALITY_JSON" \
  ...
  --max-teacher-regret "${ANCHOR_MAX_TEACHER_REGRET:-12000}" \
  2>&1 | tee "$OUT_ROOT/logs/v52_anchor_quality_gate.out"

# ---------------------------------------------------------------------------
# 1. Main v52 training. Base/local encoders are frozen by config; only the
#    reset residual/uncertainty and proposal-family heads are trainable.
# ---------------------------------------------------------------------------
if training_complete; then
  echo "[v52] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS="$GPUS" \
  V30_CKPT_IN="$V30_CKPT_IN" \
  INIT_MODE="$V52_INIT_MODE" \
  TRAIN_CONFIG=bdse/configs/v52_bfar_dbap_train_2gpu.yaml \
  ...
  bash run_v52_bfar_dbap.sh
fi

BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v52_bfar_dbap.best.pt"
[[ -s "$BEST_CHECKPOINT" ]] || { echo "Missing v52 best checkpoint: $BEST_CHECKPOINT" >&2; exit 2; }
...
calibrate_system() {
  ...
  if ! is_fresh "$json_out" "$checkpoint" "$config_in" "$CALIBRATION_PROVENANCE"; then
    ...
    echo "[v52] reuse $label calibration"
  fi
...
CAND_CAL_CFG="$OUT_ROOT/calibration/v52_bfar_dbap_candidate_calibrated.yaml"
LOCAL_CAL_JSON="$OUT_ROOT/calibration/local_control_adverse.json"
LOCAL_CAL_CFG="$OUT_ROOT/calibration/v52_bfar_dbap_local_control_calibrated.yaml"
FOUND_CAL_JSON="$OUT_ROOT/calibration/foundation_control_adverse.json"
FOUND_CAL_CFG="$OUT_ROOT/calibration/v52_bfar_dbap_foundation_control_calibrated.yaml"
calibrate_system candidate bdse/configs/v52_bfar_dbap_cl.yaml "$BEST_CHECKPOINT" "$CAND_CAL_JSON" "$CAND_CAL_CFG"
...
# ---------------------------------------------------------------------------
OPEN_LOOP_JSON="$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap.json"
OPEN_LOOP_JSONL="$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap.jsonl"
if ! is_fresh "$OPEN_LOOP_JSON" "$BEST_CHECKPOINT" "$CAND_CAL_CFG" \
  ...
  V52_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$CAND_CAL_CFG" \
...
# ---------------------------------------------------------------------------
GATE_MARKER="$OUT_ROOT/open_loop/.v52_bfar_dbap_gate_passed"
if is_fresh "$GATE_MARKER" "$OPEN_LOOP_JSON" "$OPEN_LOOP_JSONL" \
    "$LOCAL_JSON" "$LOCAL_JSONL" "$FOUND_JSON" "$FOUND_JSONL" \
    "$OUT_ROOT/train/bdse_v52_bfar_dbap.train_log.jsonl" \
    bdse/tools/check_v52_bfar_dbap_gate.py; then
  echo "[v52] three-way gate previously passed"
else
  latency_gate_args=()
  if [[ "$ENFORCE_LATENCY_BEFORE_CL" == "1" ]]; then
    latency_gate_args+=(--enforce-latency)
  fi
  python -m bdse.tools.check_v52_bfar_dbap_gate \
    ...
    --train-log "$OUT_ROOT/train/bdse_v52_bfar_dbap.train_log.jsonl" \
    --latency-target-ms "${V52_LATENCY_TARGET_MS:-500}" \
    "${latency_gate_args[@]}" \
    2>&1 | tee "$OUT_ROOT/logs/v52_bfar_dbap_gate.out"
  printf 'passed\n' >"$GATE_MARKER"
fi
...
if [[ "$RUN_CLOSED_LOOP_AFTER_GATE" == "1" ]]; then
  ...
  V52_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$LOCAL_CAL_CFG" \
  bash run_v52_bfar_dbap.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$FOUNDATION_CONTROL_ROOT" RUN_MODE=cl20 \
  V52_CKPT="$FOUNDATION_CKPT" EVAL_CONFIG="$FOUND_CAL_CFG" \
  bash run_v52_bfar_dbap.sh
  ...
  echo "[v52] gate passed; closed-loop skipped because RUN_CLOSED_LOOP_AFTER_GATE=0"
fi
...
#   --config "$CAND_CAL_CFG" \
#   --checkpoint "$OUT_ROOT/train/bdse_v52_bfar_dbap.best.pt" \
#   --split public_set_test --preprocessed-dir "$BDSE_TEST_CACHE" --device cuda \
#   --output "$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap_test.json" \
#   --per-sample-output "$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap_test.jsonl"
