...
python - "$PROMOTED_SCREEN_REPORT" "CCBR_NOLEA" <<'PY_GATE'
import json,sys
p,name=sys.argv[1:]
d=json.load(open(p))
if 'winner' in d:
...
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_5_ccbr_nolea_daepc_2gpu_v1}"
export TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v64_3_5_cc_aocc_ccbr_nolea_daepc_train_2gpu.yaml}"
