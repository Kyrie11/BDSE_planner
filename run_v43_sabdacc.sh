...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v43_sabdacc_runtime_v30ckpt}
OUT_ROOT="$(python -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$OUT_ROOT")"
OPEN_ROOT="$OUT_ROOT/open_loop"; CL_ROOT="$OUT_ROOT/closed_loop"; TRAIN_ROOT="$OUT_ROOT/train"; LOG_ROOT="$OUT_ROOT/v43_logs"
mkdir -p "$OPEN_ROOT" "$CL_ROOT" "$TRAIN_ROOT" "$LOG_ROOT"
RUN_MODE=${RUN_MODE:-open_loop}
V43_CKPT=${V43_CKPT:-$V30_CKPT_IN}
...
run_open_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_open_loop \
      --config "$cfg" --checkpoint "$V43_CKPT" --split val \
...
run_open_loop() {
  [[ -f "$V43_CKPT" ]] || { echo "Missing checkpoint: $V43_CKPT" >&2; exit 2; }
  run_open_loop_one "${GPU_OL_DACC:-0}" v43_sabdacc bdse/configs/v43_bdse_sabdacc_fast_cl.yaml &
  run_open_loop_one "${GPU_OL_CONTROL:-1}" v43_mars_control bdse/configs/v43_bdse_mars_control_fast_cl.yaml &
  wait
  set +e
  gate_args=(
    "$OPEN_ROOT/open_loop_v43_sabdacc.json"
    "$OPEN_ROOT/open_loop_v43_mars_control.json"
    --write-best "$OPEN_ROOT/recommended_result_path.txt"
    --analysis-output "$OPEN_ROOT/sabdacc_stage_analysis.txt"
  )
  if [[ -n "${V42_JSONL_FOR_COMPARE:-}" && -f "${V42_JSONL_FOR_COMPARE}" ]]; then
    gate_args+=(--previous-jsonl "${V42_JSONL_FOR_COMPARE}")
  fi
  python -S -m bdse.tools.check_v43_sabdacc_gate "${gate_args[@]}" | tee "$OPEN_ROOT/runtime_gate.txt"
  status=${PIPESTATUS[0]}
  set -e
  if [[ "$status" -ne 0 && "${ENFORCE_RUNTIME_GATE:-1}" == "1" ]]; then
    echo "V43 SAB-DACC runtime gate failed; do not run closed loop or finetune." >&2
    exit "$status"
  fi
}
...
run_train() {
  ...
  # Controlled head finetune. SAB-DACC itself is runtime-only: do not invoke
  ...
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --warm-start-from "$V30_CKPT_IN" --output "$TRAIN_ROOT/bdse_v43_head_finetune.pt" --amp \
    --log-file "$LOG_ROOT/v43_finetune.jsonl" > "$LOG_ROOT/v43_finetune.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V43_CKPT" --device cuda \
...
run_closed_loop() {
  local limit="$1"
  [[ -f "$V43_CKPT" ]] || { echo "Missing checkpoint: $V43_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_DACC:-0}" v43_sabdacc bdse/configs/v43_bdse_sabdacc_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_CONTROL:-1}" v43_mars_control bdse/configs/v43_bdse_mars_control_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_FALLBACK:-0}" v43_sabdacc_fallback bdse/configs/v43_bdse_sabdacc_fallback_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v43_sabdacc_${limit}" "$CL_ROOT/v43_mars_control_${limit}" "$CL_ROOT/v43_sabdacc_fallback_${limit}" \
    --csv "$CL_ROOT/v43_${limit}_compare.csv"
}
