...
# Accept both `SKIP_TRAIN=1 bash run_v31_rhvcdsr.sh` and
# `bash run_v31_rhvcdsr.sh SKIP_TRAIN=1` to avoid accidental training.
for kv in "$@"; do
  if [[ "$kv" == *=* ]]; then export "$kv"; fi
done


# v31 / RH-VCDSR: Receding-Horizon Viability-Calibrated Decision-Sufficient Recovery
...
TRAIN_OUTPUT="$TRAIN_ROOT/bdse_v31_rhvcdsr.pt"
V31_CKPT=${V31_CKPT:-$TRAIN_ROOT/bdse_v31_rhvcdsr.best.pt}
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  echo "[train] v31 RH-VCDSR finetune from $WARM_START"
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    --config bdse/configs/v31_bdse_rhvcdsr_train.yaml \
    --split train \
    ...
    --output "$TRAIN_OUTPUT" \
    --amp \
    --log-file "$LOG_ROOT/v31_rhvcdsr_train.jsonl" \
    > "$LOG_ROOT/v31_rhvcdsr_train.out" 2>&1
else
  echo "[train] SKIP_TRAIN=1, using existing checkpoint: $V31_CKPT"
fi

# For the causal runtime-only ablation, reuse the exact v30 checkpoint with v31 runtime guard/configs:
#   SKIP_TRAIN=1 V31_CKPT=outputs_v30/train/bdse_v30_pmvrbsr.best.pt OUT_ROOT=outputs_v31_runtime_v30ckpt bash run_v31_rhvcdsr.sh
if [[ ! -f "$V31_CKPT" ]]; then
  echo "Missing v31 checkpoint: $V31_CKPT" >&2
  echo "For runtime-only check, run: SKIP_TRAIN=1 V31_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v31_runtime_v30ckpt bash run_v31_rhvcdsr.sh" >&2
  exit 1
fi
...
paths = [
    ('v31_rhvcdsr_fixed_budget', root / 'open_loop_v31_rhvcdsr_fixed_budget.json'),
    ('v31_rhvcdsr_safety_fallback', root / 'open_loop_v31_rhvcdsr_safety_fallback.json'),
    ('v31_rhvcdsr_bbr_scur', root / 'open_loop_v31_rhvcdsr_bbr_scur.json'),
    ('v31_rhvcdsr_lcb_control', root / 'open_loop_v31_rhvcdsr_lcb_control.json'),
]
keys = [
...
if [[ "${SKIP_OPEN_LOOP:-0}" != "1" ]]; then
  if [[ "${OPEN_PARALLEL4:-1}" == "1" ]]; then
    run_open_loop "${GPU_OL_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-0}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" &
    run_open_loop "${GPU_OL_BBR:-1}" v31_rhvcdsr_bbr_scur bdse/configs/v31_bdse_rhvcdsr_bbr_scur_fast_cl.yaml "$V31_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v31_rhvcdsr_lcb_control bdse/configs/v31_bdse_rhvcdsr_lcb_control_fast_cl.yaml "$V31_CKPT" &
    wait
  else
    run_open_loop "${GPU_OL_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-1}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" &
    wait
    run_open_loop "${GPU_OL_BBR:-0}" v31_rhvcdsr_bbr_scur bdse/configs/v31_bdse_rhvcdsr_bbr_scur_fast_cl.yaml "$V31_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v31_rhvcdsr_lcb_control bdse/configs/v31_bdse_rhvcdsr_lcb_control_fast_cl.yaml "$V31_CKPT" &
    wait
  fi
  print_open_loop_compare
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  if [[ "${CL_PARALLEL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-0}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" 20 &
    run_closed_loop "${GPU_CL_BBR:-1}" v31_rhvcdsr_bbr_scur bdse/configs/v31_bdse_rhvcdsr_bbr_scur_fast_cl.yaml "$V31_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v31_rhvcdsr_lcb_control bdse/configs/v31_bdse_rhvcdsr_lcb_control_fast_cl.yaml "$V31_CKPT" 20 &
    wait
  else
    run_closed_loop "${GPU_CL_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-1}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" 20 &
    wait
    run_closed_loop "${GPU_CL_BBR:-0}" v31_rhvcdsr_bbr_scur bdse/configs/v31_bdse_rhvcdsr_bbr_scur_fast_cl.yaml "$V31_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v31_rhvcdsr_lcb_control bdse/configs/v31_bdse_rhvcdsr_lcb_control_fast_cl.yaml "$V31_CKPT" 20 &
    wait
  fi
fi
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v31_rhvcdsr_fixed_budget_20" \
    "$CL_ROOT/v31_rhvcdsr_safety_fallback_20" \
    "$CL_ROOT/v31_rhvcdsr_bbr_scur_20" \
    "$CL_ROOT/v31_rhvcdsr_lcb_control_20" \
    --csv "$CL_ROOT/v31_20_compare.csv"
fi
...
if [[ "${RUN_CL50:-0}" == "1" ]]; then
  if [[ "${RUN_CL50_ALL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL50_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-0}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" 50 &
    run_closed_loop "${GPU_CL50_BBR:-1}" v31_rhvcdsr_bbr_scur bdse/configs/v31_bdse_rhvcdsr_bbr_scur_fast_cl.yaml "$V31_CKPT" 50 &
    run_closed_loop "${GPU_CL50_LCB:-1}" v31_rhvcdsr_lcb_control bdse/configs/v31_bdse_rhvcdsr_lcb_control_fast_cl.yaml "$V31_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v31_rhvcdsr_fixed_budget_50" \
      "$CL_ROOT/v31_rhvcdsr_safety_fallback_50" \
      "$CL_ROOT/v31_rhvcdsr_bbr_scur_50" \
      "$CL_ROOT/v31_rhvcdsr_lcb_control_50" \
      --csv "$CL_ROOT/v31_50_compare.csv"
  else
    run_closed_loop "${GPU_CL50_FIXED:-0}" v31_rhvcdsr_fixed_budget bdse/configs/v31_bdse_rhvcdsr_fixed_budget_fast_cl.yaml "$V31_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-1}" v31_rhvcdsr_safety_fallback bdse/configs/v31_bdse_rhvcdsr_safety_fallback_fast_cl.yaml "$V31_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v31_rhvcdsr_fixed_budget_50" \
      "$CL_ROOT/v31_rhvcdsr_safety_fallback_50" \
...
echo "  $TRAIN_ROOT/bdse_v31_rhvcdsr.best.pt"
echo "  $OPEN_ROOT/open_loop_v31_*.json"
echo "  $CL_ROOT/v31_20_compare.csv"
echo "  $LOG_ROOT/*.closed_loop_20.diag.jsonl"
echo "Runtime-only first check: export SKIP_TRAIN=1 V31_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v31_runtime_v30ckpt RUN_MODE=open_loop; bash run_v31_rhvcdsr.sh"
echo "Runtime-only CL20: export SKIP_TRAIN=1 V31_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v31_runtime_v30ckpt RUN_MODE=cl20; bash run_v31_rhvcdsr.sh"
echo "Finetune: export V30_CKPT_IN=outputs_v30/train/bdse_v30_pmvrbsr.best.pt OUT_ROOT=outputs_v31 TRAIN_MAX_SCENARIOS=12000 VAL_MAX_SCENARIOS=1000 TRAIN_EPOCHS=4 RUN_MODE=all; bash run_v31_rhvcdsr.sh"

# CL50 after finetune: export SKIP_TRAIN=1 V31_CKPT=outputs_v31/train/bdse_v31_rhvcdsr.best.pt RUN_MODE=cl50; bash run_v31_rhvcdsr.sh
