...
# v35 DICE-BDSE: Direction-Invariant Complementary Evidence planning.
...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v35_runtime_v30ckpt}
...
LOG_ROOT="$OUT_ROOT/v35_logs"
mkdir -p "$TRAIN_ROOT" "$OPEN_ROOT" "$CL_ROOT" "$LOG_ROOT"

RUN_MODE=${RUN_MODE:-open_loop}
V35_CKPT=${V35_CKPT:-$V30_CKPT_IN}
V35_MAIN_CFG=${V35_MAIN_CFG:-bdse/configs/v35_bdse_dice_balanced_fast_cl.yaml}
V34_BASELINE_JSON=${V34_BASELINE_JSON:-$ROOT_DIR/outputs_v34_runtime_v30ckpt/open_loop/open_loop_v34_abiq_action_rank.json}
...
run_runtime_gate() {
  [[ -f "$V35_CKPT" ]] || { echo "Missing checkpoint: $V35_CKPT" >&2; exit 2; }
  run_open_loop "${GPU_OL_BALANCED:-0}" v35_dice_balanced bdse/configs/v35_bdse_dice_balanced_fast_cl.yaml &
  run_open_loop "${GPU_OL_HARD:-0}" v35_dice_hard7 bdse/configs/v35_bdse_dice_hard7_fast_cl.yaml &
  run_open_loop "${GPU_OL_INTERACTION:-1}" v35_dice_soft7 bdse/configs/v35_bdse_dice_soft7_fast_cl.yaml &
  run_open_loop "${GPU_OL_ACTION:-1}" v35_dice_influence bdse/configs/v35_bdse_dice_influence_fast_cl.yaml &
  wait

  set +e
  baseline_args=()
  [[ -f "$V34_BASELINE_JSON" ]] && baseline_args=(--baseline "$V34_BASELINE_JSON")
  python -m bdse.tools.check_v35_runtime_gate \
    "$OPEN_ROOT/open_loop_v35_dice_balanced.json" \
    "$OPEN_ROOT/open_loop_v35_dice_hard7.json" \
    "$OPEN_ROOT/open_loop_v35_dice_soft7.json" \
    "$OPEN_ROOT/open_loop_v35_dice_influence.json" \
    "${baseline_args[@]}" \
...
run_train() {
  local output="$TRAIN_ROOT/bdse_v35_dice.pt"
  ...
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --log-file "$LOG_ROOT/v35_dice_train.jsonl" \
    > "$LOG_ROOT/v35_dice_train.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V35_CKPT" --device cuda \
...
run_closed_loop_group() {
  local limit="$1"
  [[ -f "$V35_CKPT" ]] || { echo "Missing checkpoint: $V35_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_MAIN:-0}" v35_dice_main "$V35_MAIN_CFG" "$limit" &
  run_closed_loop_one "${GPU_CL_SAFETY:-0}" v35_dice_safety bdse/configs/v35_bdse_dice_safety_fallback_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_BBR:-1}" v35_dice_bbr bdse/configs/v35_bdse_dice_bbr_scur_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v35_dice_main_${limit}" "$CL_ROOT/v35_dice_safety_${limit}" "$CL_ROOT/v35_dice_bbr_${limit}" \
    --csv "$CL_ROOT/v35_${limit}_compare.csv"
}
