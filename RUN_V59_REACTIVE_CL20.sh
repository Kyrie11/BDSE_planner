...
# Run publication-protocol reactive CL20 with an already frozen V59 checkpoint
# and calibrated configs.  This script never retrains and writes to a separate
# root so NR and R outputs cannot be mixed.
: "${SOURCE_OUT_ROOT:?Set SOURCE_OUT_ROOT to the completed/frozen V59 training output}"
...
CAND_CKPT="$SOURCE_OUT_ROOT/train/bdse_v59_fscip_bfar_dbap.best.pt"
CAND_CFG="$SOURCE_OUT_ROOT/calibration/v59_fscip_bfar_dbap_candidate_calibrated.yaml"
LOCAL_CFG="$SOURCE_OUT_ROOT/calibration/v59_fscip_bfar_dbap_local_control_calibrated.yaml"
FOUND_CFG="$SOURCE_OUT_ROOT/calibration/v59_fscip_bfar_dbap_foundation_control_calibrated.yaml"
...
env "${common[@]}" OUT_ROOT="$REACTIVE_OUT_ROOT" V59_CKPT="$CAND_CKPT" EVAL_CONFIG="$CAND_CFG" \
  bash run_v59_fscip_bfar_dbap.sh
env "${common[@]}" OUT_ROOT="$LOCAL_ROOT" V59_CKPT="$CAND_CKPT" EVAL_CONFIG="$LOCAL_CFG" \
  bash run_v59_fscip_bfar_dbap.sh
env "${common[@]}" OUT_ROOT="$FOUND_ROOT" V59_CKPT="$FOUNDATION_CKPT" EVAL_CONFIG="$FOUND_CFG" \
  bash run_v59_fscip_bfar_dbap.sh

python - \
  "$REACTIVE_OUT_ROOT/closed_loop/v59_fscip_bfar_dbap_20/closed_loop_combined_summary.json" \
  "$LOCAL_ROOT/closed_loop/v59_fscip_bfar_dbap_20/closed_loop_combined_summary.json" \
  "$FOUND_ROOT/closed_loop/v59_fscip_bfar_dbap_20/closed_loop_combined_summary.json" \
  "$REACTIVE_OUT_ROOT/closed_loop/v59_three_way_reactive_cl20_protocol.json" <<'PY'
import json, sys
