...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v41_prdacc_runtime_v30ckpt}
OUT_ROOT="$(python -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$OUT_ROOT")"
OPEN_ROOT="$OUT_ROOT/open_loop"; CL_ROOT="$OUT_ROOT/closed_loop"; TRAIN_ROOT="$OUT_ROOT/train"; LOG_ROOT="$OUT_ROOT/v41_logs"
mkdir -p "$OPEN_ROOT" "$CL_ROOT" "$TRAIN_ROOT" "$LOG_ROOT"
RUN_MODE=${RUN_MODE:-open_loop}
V41_CKPT=${V41_CKPT:-$V30_CKPT_IN}
...
run_open_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_open_loop \
      --config "$cfg" --checkpoint "$V41_CKPT" --split val \
...
run_open_loop() {
  [[ -f "$V41_CKPT" ]] || { echo "Missing checkpoint: $V41_CKPT" >&2; exit 2; }
  run_open_loop_one "${GPU_OL_DACC:-0}" v41_prdacc bdse/configs/v41_bdse_prdacc_fast_cl.yaml &
  run_open_loop_one "${GPU_OL_CONTROL:-1}" v41_mars_control bdse/configs/v41_bdse_mars_control_fast_cl.yaml &
  wait
  set +e
  python -m bdse.tools.check_v41_prdacc_gate \
    "$OPEN_ROOT/open_loop_v41_prdacc.json" \
    "$OPEN_ROOT/open_loop_v41_mars_control.json" \
    --write-best "$OPEN_ROOT/recommended_result_path.txt" | tee "$OPEN_ROOT/runtime_gate.txt"
  status=${PIPESTATUS[0]}
  set -e
  analysis_args=("$OPEN_ROOT/open_loop_v41_prdacc.jsonl" --output "$OPEN_ROOT/prdacc_beam_analysis.txt")
  if [[ -n "${V40_JSONL_FOR_COMPARE:-}" && -f "${V40_JSONL_FOR_COMPARE}" ]]; then
    analysis_args+=(--v40-jsonl "${V40_JSONL_FOR_COMPARE}")
  fi
  python -m bdse.tools.analyze_v41_prdacc "${analysis_args[@]}"
  if [[ "$status" -ne 0 && "${ENFORCE_RUNTIME_GATE:-1}" == "1" ]]; then
    echo "V41 PR-DACC runtime gate failed; do not run closed loop or finetune." >&2
    exit "$status"
  fi
}
...
run_train() {
  ...
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --warm-start-from "$V30_CKPT_IN" --output "$TRAIN_ROOT/bdse_v41_head_finetune.pt" --amp \
    --log-file "$LOG_ROOT/v41_finetune.jsonl" > "$LOG_ROOT/v41_finetune.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V41_CKPT" --device cuda \
...
run_closed_loop() {
  local limit="$1"
  [[ -f "$V41_CKPT" ]] || { echo "Missing checkpoint: $V41_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_DACC:-0}" v41_prdacc bdse/configs/v41_bdse_prdacc_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_CONTROL:-1}" v41_mars_control bdse/configs/v41_bdse_mars_control_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_FALLBACK:-0}" v41_prdacc_fallback bdse/configs/v41_bdse_prdacc_fallback_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v41_prdacc_${limit}" "$CL_ROOT/v41_mars_control_${limit}" "$CL_ROOT/v41_prdacc_fallback_${limit}" \
    --csv "$CL_ROOT/v41_${limit}_compare.csv"
}
