...
: "${AF_BDMU_SCREEN_REPORT:?Set AF_BDMU_SCREEN_REPORT to the promoted V64.3.9 screen report}"
for p in "$DARM_DBR_CKPT" "$DARM_DBR_FULL_REPORT" "$AF_BDMU_SCREEN_REPORT"; do [[ -s "$p" ]] || { echo "missing required artifact: $p" >&2; exit 2; }; done
python - "$DARM_DBR_FULL_REPORT" "$AF_BDMU_SCREEN_REPORT" <<'PY'
import json,sys
for name,p in [('DARM+DBR full',sys.argv[1]),('AF-BDMU screen',sys.argv[2])]:
    r=json.load(open(p))
    if not r.get('full_promotion', False):
        raise SystemExit(f'{name} did not meet promotion criteria; AF-BDMU full training blocked.')
PY
...
python -m bdse.tools.check_v64_3_9_af_bdmu_screen \
  ...
  --output "$OUT_ROOT/provenance/v64_3_9_af_bdmu_full_audit.json"
echo "[v64.3.9] full report: $OUT_ROOT/provenance/v64_3_9_af_bdmu_full_audit.json"
