...
# Accept both `SKIP_TRAIN=1 bash run_v29_cdrbsr.sh` and
# `bash run_v29_cdrbsr.sh SKIP_TRAIN=1` to avoid accidental training.
for kv in "$@"; do
  if [[ "$kv" == *=* ]]; then export "$kv"; fi
done


# v29 / CDR-RBSR: Certificate-Drivable Risk-Bounded Safety Recovery
...
TRAIN_OUTPUT="$TRAIN_ROOT/bdse_v29_cdrbsr.pt"
V29_CKPT=${V29_CKPT:-$TRAIN_ROOT/bdse_v29_cdrbsr.best.pt}
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  echo "[train] v29 CDR-RBSR finetune from $WARM_START"
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    --config bdse/configs/v29_bdse_cdrbsr_train.yaml \
    --split train \
    ...
    --output "$TRAIN_OUTPUT" \
    --amp \
    --log-file "$LOG_ROOT/v29_cdrbsr_train.jsonl" \
    > "$LOG_ROOT/v29_cdrbsr_train.out" 2>&1
else
...
#   SKIP_TRAIN=1 V29_CKPT=outputs_v25/train/bdse_v25_dgcace.best.pt bash run_v29_cdrbsr.sh
if [[ ! -f "$V29_CKPT" ]]; then
  echo "Missing v28 checkpoint: $V29_CKPT" >&2
  echo "For runtime-only check, run: SKIP_TRAIN=1 V29_CKPT=$V25_CKPT_IN bash run_v29_cdrbsr.sh" >&2
  exit 1
fi
...
paths = [
    ('v29_cdrbsr_fixed_budget', root / 'open_loop_v29_cdrbsr_fixed_budget.json'),
    ('v29_cdrbsr_safety_fallback', root / 'open_loop_v29_cdrbsr_safety_fallback.json'),
    ('v29_cdrbsr_bbr_scur', root / 'open_loop_v29_cdrbsr_bbr_scur.json'),
    ('v29_cdrbsr_lcb_control', root / 'open_loop_v29_cdrbsr_lcb_control.json'),
]
keys = [
...
# Run four open-loop evaluations concurrently by default: two jobs on GPU 0 and two jobs on GPU 1.
# Set OPEN_PARALLEL4=0 to use the old two-at-a-time schedule.
if [[ "${OPEN_PARALLEL4:-1}" == "1" ]]; then
  run_open_loop "${GPU_OL_FIXED:-0}" v29_cdrbsr_fixed_budget bdse/configs/v29_bdse_cdrbsr_fixed_budget_fast_cl.yaml "$V29_CKPT" &
  run_open_loop "${GPU_OL_SAFETY:-0}" v29_cdrbsr_safety_fallback bdse/configs/v29_bdse_cdrbsr_safety_fallback_fast_cl.yaml "$V29_CKPT" &
  run_open_loop "${GPU_OL_BBR:-1}" v29_cdrbsr_bbr_scur bdse/configs/v29_bdse_cdrbsr_bbr_scur_fast_cl.yaml "$V29_CKPT" &
  run_open_loop "${GPU_OL_LCB:-1}" v29_cdrbsr_lcb_control bdse/configs/v29_bdse_cdrbsr_lcb_control_fast_cl.yaml "$V29_CKPT" &
  wait
else
  run_open_loop "${GPU_OL_FIXED:-0}" v29_cdrbsr_fixed_budget bdse/configs/v29_bdse_cdrbsr_fixed_budget_fast_cl.yaml "$V29_CKPT" &
  run_open_loop "${GPU_OL_SAFETY:-1}" v29_cdrbsr_safety_fallback bdse/configs/v29_bdse_cdrbsr_safety_fallback_fast_cl.yaml "$V29_CKPT" &
  wait
  run_open_loop "${GPU_OL_BBR:-0}" v29_cdrbsr_bbr_scur bdse/configs/v29_bdse_cdrbsr_bbr_scur_fast_cl.yaml "$V29_CKPT" &
  run_open_loop "${GPU_OL_LCB:-1}" v29_cdrbsr_lcb_control bdse/configs/v29_bdse_cdrbsr_lcb_control_fast_cl.yaml "$V29_CKPT" &
  wait
fi
print_open_loop_compare
...
# Preferred explicit mode to avoid the old RUN_CL50 ambiguity.
# RUN_STAGE=open_loop: open loop only; RUN_STAGE=cl20: CL20 only;
# RUN_STAGE=cl50: CL50 only; RUN_STAGE=all: CL20 + CL50.
RUN_STAGE=${RUN_STAGE:-}
if [[ -n "$RUN_STAGE" ]]; then
  case "$RUN_STAGE" in
    open_loop) export OPEN_LOOP_ONLY=1; export RUN_CL20=0; export RUN_CL50=0 ;;
    cl20) export OPEN_LOOP_ONLY=0; export RUN_CL20=1; export RUN_CL50=0 ;;
    cl50) export OPEN_LOOP_ONLY=0; export RUN_CL20=0; export RUN_CL50=1 ;;
    all) export OPEN_LOOP_ONLY=0; export RUN_CL20=1; export RUN_CL50=1 ;;
    *) echo "Unknown RUN_STAGE=$RUN_STAGE; use open_loop|cl20|cl50|all" >&2; exit 2 ;;
  esac
fi
# Backward compatibility with the observed legacy behavior: when RUN_CL20 is
# explicitly off and RUN_CL50 is explicitly 0, treat that as a CL50-only request.
# Preferred usage is RUN_STAGE=cl50.
if [[ "${RUN_CL20:-1}" == "0" && "${RUN_CL50:-unset}" == "0" && -z "${RUN_STAGE:-}" ]]; then
  export RUN_CL50=1
fi

if [[ "${RUN_CL20:-1}" == "1" ]]; then
  if [[ "${CL_PARALLEL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL_FIXED:-0}" v29_cdrbsr_fixed_budget bdse/configs/v29_bdse_cdrbsr_fixed_budget_fast_cl.yaml "$V29_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-0}" v29_cdrbsr_safety_fallback bdse/configs/v29_bdse_cdrbsr_safety_fallback_fast_cl.yaml "$V29_CKPT" 20 &
    run_closed_loop "${GPU_CL_BBR:-1}" v29_cdrbsr_bbr_scur bdse/configs/v29_bdse_cdrbsr_bbr_scur_fast_cl.yaml "$V29_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v29_cdrbsr_lcb_control bdse/configs/v29_bdse_cdrbsr_lcb_control_fast_cl.yaml "$V29_CKPT" 20 &
    wait
  else
    run_closed_loop "${GPU_CL_FIXED:-0}" v29_cdrbsr_fixed_budget bdse/configs/v29_bdse_cdrbsr_fixed_budget_fast_cl.yaml "$V29_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-1}" v29_cdrbsr_safety_fallback bdse/configs/v29_bdse_cdrbsr_safety_fallback_fast_cl.yaml "$V29_CKPT" 20 &
    wait
    run_closed_loop "${GPU_CL_BBR:-0}" v29_cdrbsr_bbr_scur bdse/configs/v29_bdse_cdrbsr_bbr_scur_fast_cl.yaml "$V29_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v29_cdrbsr_lcb_control bdse/configs/v29_bdse_cdrbsr_lcb_control_fast_cl.yaml "$V29_CKPT" 20 &
    wait
  fi
fi
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v29_cdrbsr_fixed_budget_20" \
    "$CL_ROOT/v29_cdrbsr_safety_fallback_20" \
    "$CL_ROOT/v29_cdrbsr_bbr_scur_20" \
    "$CL_ROOT/v29_cdrbsr_lcb_control_20" \
...
if [[ "${RUN_CL50:-0}" == "1" ]]; then
  run_closed_loop "${GPU_CL50_FIXED:-0}" v29_cdrbsr_fixed_budget bdse/configs/v29_bdse_cdrbsr_fixed_budget_fast_cl.yaml "$V29_CKPT" 50 &
  run_closed_loop "${GPU_CL50_SAFETY:-1}" v29_cdrbsr_safety_fallback bdse/configs/v29_bdse_cdrbsr_safety_fallback_fast_cl.yaml "$V29_CKPT" 50 &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v29_cdrbsr_fixed_budget_50" \
    "$CL_ROOT/v29_cdrbsr_safety_fallback_50" \
...
echo "  $TRAIN_ROOT/bdse_v29_cdrbsr.best.pt"
echo "  $OPEN_ROOT/open_loop_v28_*.json"
echo "  $CL_ROOT/v29_20_compare.csv"
echo "  $LOG_ROOT/*.closed_loop_20.diag.jsonl"
echo "Runtime-only first check: export SKIP_TRAIN=1 V29_CKPT=$V25_CKPT_IN RUN_STAGE=open_loop; bash run_v29_cdrbsr.sh"
echo "Runtime-only CL20: export SKIP_TRAIN=1 V29_CKPT=$V25_CKPT_IN RUN_STAGE=cl20; bash run_v29_cdrbsr.sh"
echo "Finetune: export TRAIN_MAX_SCENARIOS=12000 VAL_MAX_SCENARIOS=1000 TRAIN_EPOCHS=5; bash run_v29_cdrbsr.sh"
