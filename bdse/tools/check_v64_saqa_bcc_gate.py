...
def main() -> int:
    p = argparse.ArgumentParser(description="Three-tier V64 SAQA-BCC open-loop gate")
    ...
    dense_query_contract_available = val(cand, "dense_runtime_raw_query_feature_contract_available")
    dense_query_contract_pass = val(cand, "dense_runtime_raw_query_feature_contract_pass")
    if not math.isfinite(dense_query_contract_pass):
        dense_query_contract_pass = val(cand, "dense_runtime_query_contract_pass")
    dense_query_allclose_fraction = val(cand, "dense_runtime_raw_query_feature_allclose_fraction")
    dense_query_score_contract_pass = val(cand, "dense_runtime_query_score_contract_pass")
    dense_query_score_allclose_fraction = val(cand, "dense_runtime_query_score_allclose_fraction")
    dense_query_decision_match = val(cand, "dense_runtime_query_decision_match")
    query_legacy_support_dim = val(cand, "query_legacy_support_dim")
    query_extension_dim = val(cand, "query_extension_dim")
    query_extension_adapter_enabled = val(cand, "query_extension_adapter_enabled")
    ...
    protocol_checks = [
        ...
        (math.isfinite(query_legacy_support_dim) and abs(query_legacy_support_dim - 12.0) <= 1e-6, f"query legacy support dim={query_legacy_support_dim}, expected 12"),
        (math.isfinite(query_extension_dim) and abs(query_extension_dim - 6.0) <= 1e-6, f"query extension dim={query_extension_dim}, expected 6"),
        (math.isfinite(query_extension_adapter_enabled) and query_extension_adapter_enabled >= 0.99, f"query extension adapter inactive: {query_extension_adapter_enabled}"),
        (math.isfinite(dense_query_contract_available) and dense_query_contract_available >= 0.99, f"dense/runtime raw query-feature contract unavailable: {dense_query_contract_available}"),
        (math.isfinite(dense_query_contract_pass) and dense_query_contract_pass >= 0.99, f"dense/runtime raw query-feature contract pass={dense_query_contract_pass} < 0.99"),
        (math.isfinite(dense_query_allclose_fraction) and dense_query_allclose_fraction >= 0.999, f"dense/runtime raw query-feature allclose fraction={dense_query_allclose_fraction} < 0.999"),
        (math.isfinite(dense_query_score_contract_pass) and dense_query_score_contract_pass >= 0.99, f"dense/runtime query-score numerical contract pass={dense_query_score_contract_pass} < 0.99"),
        (math.isfinite(dense_query_score_allclose_fraction) and dense_query_score_allclose_fraction >= 0.999, f"dense/runtime query-score allclose fraction={dense_query_score_allclose_fraction} < 0.999"),
        (math.isfinite(dense_query_decision_match) and dense_query_decision_match >= 0.999, f"dense/runtime query decision match={dense_query_decision_match} < 0.999"),
    ...
    report = {
        ...
        "metrics": {
            ...
            "query_legacy_support_dim": query_legacy_support_dim,
            "query_extension_dim": query_extension_dim,
            "query_extension_adapter_enabled": query_extension_adapter_enabled,
            "dense_runtime_raw_query_feature_contract_available": dense_query_contract_available,
            "dense_runtime_raw_query_feature_contract_pass": dense_query_contract_pass,
            "dense_runtime_raw_query_feature_allclose_fraction": dense_query_allclose_fraction,
            "dense_runtime_query_score_contract_pass": dense_query_score_contract_pass,
            "dense_runtime_query_score_allclose_fraction": dense_query_score_allclose_fraction,
            "dense_runtime_query_decision_match": dense_query_decision_match,
    ...
    print(f"\nV64 protocol-integrity gate [{'PASS' if protocol_pass else 'FAIL'}]")
    ...
    print(f"V64 minimum-completeness gate [{min_label}]")
    print(f"V64 competitive gate [{comp_label}]")
