...
GPUS="${GPUS:-0,1}"
SMOKE_OUT_ROOT="${SMOKE_OUT_ROOT:-outputs_v61_dehab_training_smoke}"
SMOKE_TRAIN_SCENARIOS="${SMOKE_TRAIN_SCENARIOS:-1024}"
SMOKE_VAL_SCENARIOS="${SMOKE_VAL_SCENARIOS:-256}"
BASE_CONFIG="${BASE_CONFIG:-bdse/configs/v61_dehab_bfar_dbap_train_2gpu.yaml}"
SMOKE_CONFIG="$SMOKE_OUT_ROOT/v61_dehab_bfar_dbap_smoke_2gpu.yaml"
...
train["name"] = "v61_dehab_bfar_dbap_training_smoke"
dst.parent.mkdir(parents=True, exist_ok=True)
dst.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
PY

rm -rf "$SMOKE_OUT_ROOT/train" "$SMOKE_OUT_ROOT/logs" "$SMOKE_OUT_ROOT/.v61_run.lock"
...
LOG="$SMOKE_OUT_ROOT/train/bdse_v61_dehab_bfar_dbap.train_log.jsonl"
...
if not rows:
    raise SystemExit("V61 smoke failed: empty training log")
...
if failed:
    raise SystemExit("V61 smoke failed: " + ", ".join(failed))
print("V61 training smoke: PASS")
PY
