...
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_48_2_eaf_icer_ocrr_screen_2gpu_v1}"
...
export FRESH_HASH_SEED="${FRESH_HASH_SEED:-v64.3.48.2-eaf-icer-ocrr-double-fresh-v1}"
...
TIMING="$OUT_ROOT/provenance/v64_3_48_2_stage_timing.tsv"; :>"$TIMING"; ALL_START=$(date +%s)
stage_start(){ STAGE_NAME="$1"; STAGE_START=$(date +%s); }
stage_end(){ printf '%s\t%s\n' "$STAGE_NAME" "$(( $(date +%s)-STAGE_START ))" >>"$TIMING"; }

for f in "$RAW_CONFIG" "$V20_CONFIG" "$V44_PRESERVE" "$V47_FIT" "$V47_AUDIT" "$V47_PLAN" "$V47_EGO" "$DESIGN_EXCLUDE_TOKENS" "$FROZEN_TRAIN_TOKENS" "$V48_CONSUMED_FRESH_TOKENS" V64_3_48_2_SOURCE_MANIFEST.sha256 V64_3_48_OCRR_SCIENCE_LOCK.sha256; do
  [[ -s "$f" ]] || { echo "STOP V48.2 missing $f" >&2; exit 2; }
done
[[ -d "$BDSE_VAL_CACHE" ]] || { echo 'STOP V48.2 missing val cache' >&2; exit 2; }

stage_start source_and_v47_provenance_gate
sha256sum -c V64_3_48_2_SOURCE_MANIFEST.sha256 | tee "$OUT_ROOT/logs/v64_3_48_2_source_manifest.out"
sha256sum -c V64_3_48_OCRR_SCIENCE_LOCK.sha256 | tee "$OUT_ROOT/logs/v64_3_48_2_ocrr_science_lock.out"
python - "$V48_CONSUMED_FRESH_TOKENS" <<'PY48TOK'
import hashlib,sys
p=sys.argv[1]
xs=[x.strip() for x in open(p) if x.strip()]
expected='c09becde1bca5a8d2d6fe2166980c4bb1a0923b02fdd69d598c5e1e3d4fc6d2e'
if len(xs)!=1000 or len(set(xs))!=1000:
    raise SystemExit('STOP V48.2: consumed V48 fresh token ledger must contain exactly 1000 unique tokens')
got=hashlib.sha256(open(p,'rb').read()).hexdigest()
if got!=expected:
    raise SystemExit(f'STOP V48.2: consumed fresh ledger identity changed: {got}')
print('PASS V48.2 consumed-fresh ledger: 1000/1000 unique and byte-locked')
PY48TOK
...
for p,h in expected.items():
 ...
 if g!=h: raise SystemExit(f'STOP V48.2: V47 prerequisite byte identity changed: {p} {g}')
...
for k,x in exp.items():
 ...
 if any(g[i]!=x[i] for i in range(4)) or abs(g[4]-x[4])>1e-9: raise SystemExit(f'STOP V48.2: V47 signature changed {k}: {g}')
if r.get('train_gate_pass') is not False or n.get('train_gate_pass') is not False:
 raise SystemExit('STOP V48.2: V47 unexpectedly passed TRAIN gate')
...
if not bool(ident.get('agent_2d_response_identified',False)) or not bool(ident.get('ego_reference_identified',False)):
 raise SystemExit('STOP V48.2: V47 nuisance identification changed')
if n.get('failure_diagnosis')!='future_state_nuisances_are_identifiable_but_absolute_zero_requires_selected_deployment_decision_functional_or_more_general_future_state':
 raise SystemExit('STOP V48.2: V47 scientific-stop signature changed')
...
if len(tr)!=3000 or len(set(tr))!=3000 or hashlib.sha256(train.read_bytes()).hexdigest()!='b36a847e7a3d7caa3c785ac96b6789ddefed071fae050170482108d950447da4':
 raise SystemExit('STOP V48.2: frozen TRAIN changed')
...
if any(p.exists() and p.stat().st_size>0 for p in spent): raise SystemExit('STOP V48.2: V47 fresh was consumed')
print('PASS V48.2 prerequisite: exact V47 bytes, preregistered representation-stop, and prior V48 fresh is recorded as consumed; OCRR science is byte-locked for clean rerun')
PY
cp V64_3_48_2_SOURCE_MANIFEST.sha256 "$OUT_ROOT/provenance/"
cp V64_3_48_OCRR_SCIENCE_LOCK.sha256 "$OUT_ROOT/provenance/"
cp "$V48_CONSUMED_FRESH_TOKENS" "$OUT_ROOT/provenance/v64_3_48_consumed_fresh1000_tokens.txt"
stage_end

stage_start prerequisites_and_regression
python -m bdse.tools.check_v64_3_13_eaf_dmvr_screen --train-log "$EAF_TRAIN_LOG" --variant REAUDIT_FOR_V64_3_48_2_OCRR --output "$OUT_ROOT/provenance/v64_3_13_eaf_dmvr_reaudit_for_v64_3_48_2.json" > "$OUT_ROOT/logs/v64_3_13_reaudit_v64_3_48_2.out"
read -r SELECTED_EPOCH < <(python - "$OUT_ROOT/provenance/v64_3_13_eaf_dmvr_reaudit_for_v64_3_48_2.json" <<'PY'
import json,sys
r=json.load(open(sys.argv[1]))
if not r.get('training_instrumentation_valid',False) or not r.get('acquisition_frozen',False) or not r.get('value_estimation_gain',False): raise SystemExit('STOP V48.2 prerequisites')
...
export EAF_CKPT; [[ -s "$EAF_CKPT" ]] || { echo 'STOP V48.2 missing EAF checkpoint' >&2; exit 2; }
...
NOMULT_CONFIG="$OUT_ROOT/provenance/v64_3_48_2_sign_nomult.yaml"
MULT_CONFIG="$OUT_ROOT/provenance/v64_3_48_2_sign_mult.yaml"
FIT_REPORT="$OUT_ROOT/provenance/v64_3_48_2_ocrr_fit.json"
SCENE_AUDIT="$OUT_ROOT/provenance/v64_3_48_2_ocrr_train_scene_audit.csv"
set +e
python -m bdse.tools.fit_v64_3_48_eaf_icer_ocrr \
  ...
  --output-report "$FIT_REPORT" --output-scene-audit "$SCENE_AUDIT" 2>&1 | tee "$OUT_ROOT/logs/v64_3_48_2_ocrr_fit.out"
...
[[ "$PREFERRED" =~ ^(sign_nomult|sign_mult)$ ]] || { echo 'STOP V48.2 no preregistered preferred arm' >&2; exit 2; }

stage_start double_fresh_selection
TOK1000="$OUT_ROOT/provenance/val_screen_fresh1000_tokens.txt"; TOKA="$OUT_ROOT/provenance/val_screen_fresh_A_tokens.txt"; TOKB="$OUT_ROOT/provenance/val_screen_fresh_B_tokens.txt"; EX="$OUT_ROOT/provenance/v64_3_48_2_selection_exclude.txt"
cat "$DESIGN_EXCLUDE_TOKENS" "$FROZEN_TRAIN_TOKENS" "$V48_CONSUMED_FRESH_TOKENS" | awk 'NF&&!seen[$0]++' > "$EX"
python -m bdse.tools.select_fresh_preprocessed_tokens --preprocessed-dir "$BDSE_VAL_CACHE" --split val --exclude-tokens "$EX" --count 1000 --hash-seed "$FRESH_HASH_SEED" --output "$TOK1000" --audit-output "$OUT_ROOT/provenance/v64_3_48_2_fresh1000_selection_audit.json" > "$OUT_ROOT/logs/fresh_selection.out"
...
if len(A)!=500 or len(B)!=500 or len(set(A))!=500 or len(set(B))!=500 or set(A)&set(B) or (set(A)|set(B))&E: raise SystemExit('STOP V48.2 fresh independence')
print('PASS V48.2 independent A500+B500 selection')
PY
stage_end
...
for sp in A B; do
 ...
 python -m bdse.tools.check_v64_3_48_eaf_icer_ocrr_split --split-name "$sp" --preferred-arm "$PREFERRED" "${args[@]}" --output "$OUT_ROOT/provenance/v64_3_48_2_split_${sp}_screen.json" | tee "$OUT_ROOT/logs/v64_3_48_2_split_${sp}.out"
done
python -m bdse.tools.check_v64_3_48_eaf_icer_ocrr_screen --split-a "$OUT_ROOT/provenance/v64_3_48_2_split_A_screen.json" --split-b "$OUT_ROOT/provenance/v64_3_48_2_split_B_screen.json" --fit-report "$FIT_REPORT" --output "$OUT_ROOT/provenance/v64_3_48_2_eaf_icer_ocrr_double_fresh_screen.json" | tee "$OUT_ROOT/logs/v64_3_48_2_screen.out"
stage_end
