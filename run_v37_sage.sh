...
# v37 SAGE-BDSE: Safety-Always-on Graded Evidence planning.
...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v37_runtime_v30ckpt}
...
LOG_ROOT="$OUT_ROOT/v37_logs"
mkdir -p "$TRAIN_ROOT" "$OPEN_ROOT" "$CL_ROOT" "$LOG_ROOT"

RUN_MODE=${RUN_MODE:-open_loop}
V37_CKPT=${V37_CKPT:-$V30_CKPT_IN}
V37_MAIN_CFG=${V37_MAIN_CFG:-bdse/configs/v37_bdse_sage_balanced_fast_cl.yaml}
...
run_runtime_gate() {
  [[ -f "$V37_CKPT" ]] || { echo "Missing checkpoint: $V37_CKPT" >&2; exit 2; }
  run_open_loop "${GPU_OL_BALANCED:-0}" v37_sage_balanced bdse/configs/v37_bdse_sage_balanced_fast_cl.yaml &
  run_open_loop "${GPU_OL_RISK:-0}" v37_sage_risk35 bdse/configs/v37_bdse_sage_risk35_fast_cl.yaml &
  run_open_loop "${GPU_OL_FULL:-1}" v37_sage_full_graph bdse/configs/v37_bdse_sage_full_graph_fast_cl.yaml &
  run_open_loop "${GPU_OL_INTERACTION:-1}" v37_sage_interaction7 bdse/configs/v37_bdse_sage_interaction7_fast_cl.yaml &
  wait

  set +e
  baseline_args=()
  [[ -f "$V35_BASELINE_JSON" ]] && baseline_args=(--baseline "$V35_BASELINE_JSON")
  python -m bdse.tools.check_v37_runtime_gate \
    "$OPEN_ROOT/open_loop_v37_sage_balanced.json" \
    "$OPEN_ROOT/open_loop_v37_sage_risk35.json" \
    "$OPEN_ROOT/open_loop_v37_sage_full_graph.json" \
    "$OPEN_ROOT/open_loop_v37_sage_interaction7.json" \
    "${baseline_args[@]}" \
...
run_train() {
  ...
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    ...
    --log-file "$LOG_ROOT/v37_sage_train.jsonl" \
    > "$LOG_ROOT/v37_sage_train.out" 2>&1
}
...
run_closed_loop_one() {
  ...
  (
    ...
    python -m bdse.experiments.evaluate_closed_loop \
      --config "$cfg" --checkpoint "$V37_CKPT" --device cuda \
...
run_closed_loop_group() {
  local limit="$1"
  [[ -f "$V37_CKPT" ]] || { echo "Missing checkpoint: $V37_CKPT" >&2; exit 2; }
  run_closed_loop_one "${GPU_CL_MAIN:-0}" v37_sage_main "$V37_MAIN_CFG" "$limit" &
  run_closed_loop_one "${GPU_CL_SAFETY:-0}" v37_sage_safety bdse/configs/v37_bdse_sage_safety_fallback_fast_cl.yaml "$limit" &
  run_closed_loop_one "${GPU_CL_BBR:-1}" v37_sage_bbr bdse/configs/v37_bdse_sage_bbr_scur_fast_cl.yaml "$limit" &
  wait
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v37_sage_main_${limit}" "$CL_ROOT/v37_sage_safety_${limit}" "$CL_ROOT/v37_sage_bbr_${limit}" \
    --csv "$CL_ROOT/v37_${limit}_compare.csv"
}
