...
: "${V57_CHECKPOINT:?Set V57_CHECKPOINT to the already frozen V57 checkpoint}"
: "${V57_TEST_FROZEN_ACK:?Set V57_TEST_FROZEN_ACK=YES after freezing checkpoint/config/gates}"
[[ "$V57_TEST_FROZEN_ACK" == "YES" ]] || { echo "Refusing test evaluation: set V57_TEST_FROZEN_ACK=YES only after all tuning is frozen." >&2; exit 2; }

GPUS="${GPUS:-0,1}"
TEST_CONFIG="${TEST_CONFIG:-bdse/configs/v57_wcdcip_bfar_dbap_cl.yaml}"
TEST_OUT_ROOT="${TEST_OUT_ROOT:-outputs_v57_wcdcip_bfar_partial_test_frozen}"
...
python - "$READINESS" "$V57_CHECKPOINT" "$TEST_CONFIG" "$TEST_OUT_ROOT/provenance/frozen_test_protocol.json" <<'PY'
