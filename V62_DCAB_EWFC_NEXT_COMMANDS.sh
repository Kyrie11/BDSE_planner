...
# v62.0 checkpoint-independent pipeline.  Resolve every relative path from the
# checked-in script directory so a stale working directory cannot silently run a
# different config or helper script.
PIPELINE_VERSION="v62.1-deployment-complete-action-bridge-exact-winner-flip-criticality"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
echo "[v62] pipeline_version=$PIPELINE_VERSION script=$SCRIPT_DIR/$(basename "$0")"
...
# alias consumed by run_v62_dcab_ewfc.sh.
...
export V62_INIT_MODE="${V62_INIT_MODE:-warm_start}"
...
export FOUNDATION_CONTROL_CONFIG="${FOUNDATION_CONTROL_CONFIG:-bdse/configs/v62_dcab_ewfc_anchor_control_cl.yaml}"
export LOCAL_CONTROL_CONFIG="${LOCAL_CONTROL_CONFIG:-bdse/configs/v62_dcab_ewfc_local_control_cl.yaml}"
...
# run_v62_dcab_ewfc.sh lets this parent continue immediately into calibration while
...
if [[ "$PIPELINE_DETACH" == "1" && "${BDSE_PIPELINE_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v62] detached pipeline started pid=$pipeline_pid log=$pipeline_log"
  exit 0
fi

PIPELINE_LOCK_DIR="$OUT_ROOT/.v62_pipeline.lock"
if ! mkdir "$PIPELINE_LOCK_DIR" 2>/dev/null; then
  owner="$(cat "$PIPELINE_LOCK_DIR/pid" 2>/dev/null || echo unknown)"
  echo "A v62 pipeline is already using OUT_ROOT=$OUT_ROOT (pid=$owner)" >&2
  exit 3
fi
...
training_complete() {
  ...
  [[ -s "$OUT_ROOT/train/bdse_v62_dcab_ewfc.train_log.jsonl"      && -s "$OUT_ROOT/train/bdse_v62_dcab_ewfc.pt"      && -s "$OUT_ROOT/train/bdse_v62_dcab_ewfc.best.pt"      && "$OUT_ROOT/train/bdse_v62_dcab_ewfc.pt" -nt bdse/configs/v62_dcab_ewfc_train_2gpu.yaml ]]
}
...
if [[ "$PIPELINE_FORCE" != "1" \
  ...
  echo "[v62] stage 0 already complete: reuse group-disjoint split"
else
...
# They are algorithm-specific initializations and would confound the v62 paper
...
resolve_foundation_checkpoint() {
  ...
  if [[ -n "$FOUNDATION_CKPT" && ! -s "$FOUNDATION_CKPT" ]]; then
    echo "[v62] ignore missing inherited foundation path: $FOUNDATION_CKPT"
    FOUNDATION_CKPT=""
  fi
  ...
  if [[ "$FOUNDATION_POLICY" != "rebuild" && "$RECOVER_SAFE_FOUNDATION_COPIES" == "1" ]]; then
    ...
    if [[ "$ALLOW_ALGORITHM_CHECKPOINT_INIT" == "1" ]]; then
      ...
      echo "[v62] WARNING: algorithm-specific checkpoint recovery enabled; use only as a transfer ablation" >&2
    fi
    selected="$(python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v62_dcab_ewfc_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY"       --print-selected       "${allow_args[@]}")"
    if [[ -n "$selected" && -s "$selected" ]]; then
      ...
      echo "[v62] recovered foundation checkpoint=$FOUNDATION_CKPT source=$FOUNDATION_SOURCE"
      return 0
    fi
  else
    ...
    python -m bdse.tools.resolve_foundation_checkpoint       --config bdse/configs/v62_dcab_ewfc_train_2gpu.yaml       --search-root "$FOUNDATION_SEARCH_ROOT"       --output-json "$FOUNDATION_INVENTORY" >/dev/null || true
  fi
...
if [[ "$V62_INIT_MODE" == "warm_start" ]]; then
  resolve_foundation_checkpoint
  ...
  echo "[v62] V62_INIT_MODE=scratch is an ablation, not the paper main run."
  [[ -n "$CONTROL_CKPT" && -s "$CONTROL_CKPT" ]] || {
    echo "Scratch v62 still needs an independently trained matched CONTROL_CKPT for a paired gate" >&2
    exit 2
  }
...
echo "[v62] foundation_source=$FOUNDATION_SOURCE"
echo "[v62] foundation_checkpoint=$FOUNDATION_CKPT"
echo "[v62] control_checkpoint=$CONTROL_CKPT"
...
#      uninterpretable, so the algorithm stage is blocked before any v62 update.
# ---------------------------------------------------------------------------
...
if is_fresh "$FOUNDATION_QUALITY_JSON" "$FOUNDATION_CKPT" "$FOUNDATION_CONTROL_CONFIG" \
  ...
  echo "[v62] foundation quality replay already complete"
else
...
ANCHOR_GATE_REPORT="$OUT_ROOT/provenance/v62_anchor_gate.json"
python -m bdse.tools.check_v53_anchor_quality "$FOUNDATION_QUALITY_JSON" \
  ...
  2>&1 | tee "$OUT_ROOT/logs/v62_anchor_quality_gate.out"

# ---------------------------------------------------------------------------
# 1. Main v62 training. Base/local encoders are frozen by config; only the
#    reset residual/uncertainty and proposal-family heads are trainable.
# ---------------------------------------------------------------------------
if training_complete; then
  echo "[v62] stage 1 already complete: reuse final/best checkpoints"
else
  DETACH=0 \
  GPUS="$GPUS" \
  V30_CKPT_IN="$V30_CKPT_IN" \
  INIT_MODE="$V62_INIT_MODE" \
  TRAIN_CONFIG=bdse/configs/v62_dcab_ewfc_train_2gpu.yaml \
...
BEST_CHECKPOINT="$OUT_ROOT/train/bdse_v62_dcab_ewfc.best.pt"
[[ -s "$BEST_CHECKPOINT" ]] || { echo "Missing v62 best checkpoint: $BEST_CHECKPOINT" >&2; exit 2; }
...
#    not calibrate the direct residual uncertainty used by the flip guard.  V62
...
DUAL_CAL_JSON="$OUT_ROOT/calibration/v62_dual_certificate.json"
...
if ! is_fresh "$DUAL_CAL_JSON" "$BEST_CHECKPOINT" bdse/configs/v62_dcab_ewfc_cl.yaml "$CALIBRATION_PROVENANCE"; then
  prepare_calibration_shards
  CUDA_VISIBLE_DEVICES="${GPUS%%,*}" python -m bdse.tools.calibrate_v61_dual_certificates \
    ...
    --config bdse/configs/v62_dcab_ewfc_cl.yaml --checkpoint "$BEST_CHECKPOINT" \
  ...
  echo "[v62] reuse dual-certificate calibration"
fi

CAND_CAL_CFG="$OUT_ROOT/calibration/v62_dcab_ewfc_candidate_calibrated.yaml"
LOCAL_CAL_CFG="$OUT_ROOT/calibration/v62_dcab_ewfc_local_control_calibrated.yaml"
FOUND_CAL_CFG="$OUT_ROOT/calibration/v62_dcab_ewfc_foundation_control_calibrated.yaml"
python -m bdse.tools.apply_v61_dual_calibration --config bdse/configs/v62_dcab_ewfc_cl.yaml --calibration-json "$DUAL_CAL_JSON" --output "$CAND_CAL_CFG"
...
# ---------------------------------------------------------------------------
PROTOCOL_GATE_MARKER="$OUT_ROOT/open_loop/.v62_protocol_gate_passed"
COMP_GATE_MARKER="$OUT_ROOT/open_loop/.v62_competitive_gate_passed"
GATE_REPORT="$OUT_ROOT/open_loop/v62_dcab_ewfc_gate_report.json"
latency_gate_args=()
if [[ "$ENFORCE_LATENCY_BEFORE_CL" == "1" ]]; then latency_gate_args+=(--enforce-latency); fi
python -m bdse.tools.check_v62_dcab_ewfc_gate \
  ...
  --train-log "$OUT_ROOT/train/bdse_v62_dcab_ewfc.train_log.jsonl" \
  --train-config bdse/configs/v62_dcab_ewfc_train_2gpu.yaml \
  ...
  --report-json "$GATE_REPORT" --latency-target-ms "${V62_LATENCY_TARGET_MS:-500}" \
  "${latency_gate_args[@]}" 2>&1 | tee "$OUT_ROOT/logs/v62_dcab_ewfc_gate.out"
...
[[ "$PROTOCOL_PASS" == "1" ]] || { echo "[v62] protocol gate failed; closed loop is invalid" >&2; exit 3; }
...
if [[ "$RUN_CLOSED_LOOP_AFTER_GATE" == "1" && ( "$MINIMUM_PASS" == "1" || "$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL" == "1" ) ]]; then
  ...
  if [[ "$MINIMUM_PASS" != "1" ]]; then
    echo "[v62] minimum gate failed; running paired CL20 as diagnostic evidence only"
  ...
  V62_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$CAND_CAL_CFG" \
  bash run_v62_dcab_ewfc.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$LOCAL_CONTROL_ROOT" RUN_MODE=cl20 \
  V62_CKPT="$BEST_CHECKPOINT" EVAL_CONFIG="$LOCAL_CAL_CFG" \
  bash run_v62_dcab_ewfc.sh

  BDSE_VAL_CACHE="$BDSE_SPLIT_CACHE" CL_TOKEN_SPLIT=val_tune \
  GPUS="$GPUS" OUT_ROOT="$FOUNDATION_CONTROL_ROOT" RUN_MODE=cl20 \
  V62_CKPT="$FOUNDATION_CKPT" EVAL_CONFIG="$FOUND_CAL_CFG" \
  bash run_v62_dcab_ewfc.sh
  ...
  python - \
    "$OUT_ROOT/closed_loop/v62_dcab_ewfc_20/closed_loop_combined_summary.json" \
    "$LOCAL_CONTROL_ROOT/closed_loop/v62_dcab_ewfc_20/closed_loop_combined_summary.json" \
    "$FOUNDATION_CONTROL_ROOT/closed_loop/v62_dcab_ewfc_20/closed_loop_combined_summary.json" \
    "$OUT_ROOT/closed_loop/v62_three_way_cl20_protocol.json" <<'PY_CL_PROTOCOL'
import json, sys
from pathlib import Path
...
PY_CL_PROTOCOL
  ...
  if [[ "$RUN_CL100_AFTER_CL20" == "1" && "$COMPETITIVE_PASS" == "1" ]]; then
    ...
    echo "[v62] CL100 requested but competitive gate failed; run paired CL20 first and inspect $GATE_REPORT"
  fi
else
  echo "[v62] closed-loop skipped (RUN_CLOSED_LOOP_AFTER_GATE=$RUN_CLOSED_LOOP_AFTER_GATE, minimum_pass=$MINIMUM_PASS, diagnostic=$RUN_DIAGNOSTIC_CL20_ON_GATE_FAIL)"
fi
...
#   --config "$CAND_CAL_CFG" \
#   --checkpoint "$OUT_ROOT/train/bdse_v62_dcab_ewfc.best.pt" \
#   --split public_set_test --preprocessed-dir "$BDSE_TEST_CACHE" --device cuda \
#   --output "$OUT_ROOT/open_loop/open_loop_v62_dcab_ewfc_test.json" \
#   --per-sample-output "$OUT_ROOT/open_loop/open_loop_v62_dcab_ewfc_test.jsonl"
