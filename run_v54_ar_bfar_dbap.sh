...
# Allow either:
#   GPUS=0,1 RUN_MODE=train_open_loop bash run_v54_ar_bfar_dbap.sh
# or:
#   bash run_v54_ar_bfar_dbap.sh GPUS=0,1 RUN_MODE=train_open_loop
for kv in "$@"; do
  [[ "$kv" == *=* ]] && export "$kv"
done
...
OUT_ROOT="${OUT_ROOT:-outputs_v54_ar_bfar_dbap_2gpu}"
...
EVAL_CONFIG="${EVAL_CONFIG:-bdse/configs/v54_ar_bfar_dbap_cl.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v54_ar_bfar_dbap_train_2gpu.yaml}"
...
if [[ "$DETACH" == "1" && "${BDSE_DETACHED_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v54] detached session started pid=$child_pid log=$launcher_log"
  exit 0
fi
...
RUN_LOCK_DIR="$OUT_ROOT/.v54_run.lock"
...
CKPT="$OUT_ROOT/train/bdse_v54_ar_bfar_dbap.pt"
BEST_CKPT="$OUT_ROOT/train/bdse_v54_ar_bfar_dbap.best.pt"
LATEST_CKPT="$OUT_ROOT/train/bdse_v54_ar_bfar_dbap.latest.pt"
...
for path in dict.fromkeys(candidates):
    ...
    try:
        ...
        print(f"[v54] ignore unreadable checkpoint {path}: {type(exc).__name__}: {exc}", file=sys.stderr)
        continue
...
train_2gpu() {
  ...
  if [[ -n "$resume_checkpoint" ]]; then
    ...
    echo "[v54] resume checkpoint=$resume_checkpoint"
  elif [[ "$INIT_MODE" == "warm_start" ]]; then
    check_checkpoint "$V30_CKPT_IN"
    checkpoint_args+=(--warm-start-from "$V30_CKPT_IN")
    echo "[v54] warm-start checkpoint=$V30_CKPT_IN"
  else
    echo "[v54] training from random initialization (INIT_MODE=scratch)"
  fi
  echo "[v54] DDP training on physical GPUs $GPU0,$GPU1"
  echo "[v54] batch_per_gpu=$BATCH_SIZE_PER_GPU global_batch=$effective_global_batch workers_per_gpu=$NUM_WORKERS_PER_GPU"
  echo "[v54] train_config=$TRAIN_CONFIG val_split=$VAL_SPLIT val_scenarios=$VAL_SCENARIOS val_every=$VAL_EVERY_N_EPOCHS dense_val=$VAL_DENSE_DIAGNOSTIC"
  echo "[v54] auto_resume=$AUTO_RESUME save_every_n_steps=$SAVE_EVERY_N_STEPS init_mode=$INIT_MODE"
  echo "[v54] exact_selector_cpu_backend=$EXACT_SELECTOR_CPU_BACKEND workers_per_rank=$EXACT_SELECTOR_WORKERS_PER_RANK"
  echo "[v54-ar-bfar] selected-local anchor with boundary-focused exact AOCC supervision"
  ...
  torchrun \
    ...
    -m bdse.experiments.train \
      ...
      --log-file "$OUT_ROOT/train/bdse_v54_ar_bfar_dbap.train_log.jsonl" \
      --output "$CKPT" \
...
run_open_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_open_loop \
    ...
    --device cuda \
    --output "$OUT_ROOT/open_loop/open_loop_v54_ar_bfar_dbap.gpu${shard_id}.json" \
    --per-sample-output "$OUT_ROOT/open_loop/open_loop_v54_ar_bfar_dbap.gpu${shard_id}.jsonl" \
...
summary_paths = [root / f"open_loop_v54_ar_bfar_dbap.gpu{i}.json" for i in range(2)]
row_paths = [root / f"open_loop_v54_ar_bfar_dbap.gpu{i}.jsonl" for i in range(2)]
...
(root / "open_loop_v54_ar_bfar_dbap.json").write_text(json.dumps(out, indent=2, sort_keys=True), encoding="utf-8")
with (root / "open_loop_v54_ar_bfar_dbap.jsonl").open("w", encoding="utf-8") as f:
    for row in rows:
...
open_loop_2gpu() {
  local checkpoint="${V54_CKPT:-$BEST_CKPT}"
  check_checkpoint "$checkpoint"
  local shard_root="$OUT_ROOT/open_loop/.two_gpu_shards"
  prepare_open_loop_shards "$shard_root"

  echo "[v54] Open-loop source_split=$OPEN_LOOP_SPLIT across physical GPUs $GPU0 and $GPU1"
...
run_closed_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_closed_loop \
    ...
    --experiment-uid "v54_ar_bfar_dbap_${limit}_gpu${shard_id}" \
...
closed_loop_2gpu() {
  ...
  local run_root="$OUT_ROOT/closed_loop/v54_ar_bfar_dbap_${limit}"
  local token_root="$run_root/scenario_token_shards"
  rm -rf "$run_root"
  mkdir -p "$run_root"
  prepare_closed_loop_token_shards "$limit" "$token_root"

  echo "[v54] Closed-loop token_source_split=$CL_TOKEN_SPLIT, $limit scenarios across physical GPUs $GPU0 and $GPU1"
  echo "[v54] Reuse $token_root/scenario_tokens_all.json for every compared method to keep the CL subset identical."
  local start_time end_time
  start_time=$(date +%s)
