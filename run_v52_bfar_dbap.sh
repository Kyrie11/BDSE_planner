...
# Allow either:
#   GPUS=0,1 RUN_MODE=train_open_loop bash run_v52_bfar_dbap.sh
# or:
#   bash run_v52_bfar_dbap.sh GPUS=0,1 RUN_MODE=train_open_loop
for kv in "$@"; do
  [[ "$kv" == *=* ]] && export "$kv"
done
...
FOUNDATION_CONFIG="${FOUNDATION_CONFIG:-bdse/configs/v52_factorized_anchor_fast_2gpu.yaml}"
FOUNDATION_OUT_ROOT="${FOUNDATION_OUT_ROOT:-outputs_v52_factorized_anchor}"
OUT_ROOT="${OUT_ROOT:-outputs_v52_bfar_dbap_2gpu}"
...
EVAL_CONFIG="${EVAL_CONFIG:-bdse/configs/v52_bfar_dbap_cl.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-bdse/configs/v52_bfar_dbap_train_2gpu.yaml}"
...
if [[ "$DETACH" == "1" && "${BDSE_DETACHED_CHILD:-0}" != "1" ]]; then
  ...
  echo "[v52] detached session started pid=$child_pid log=$launcher_log"
  exit 0
fi
...
RUN_LOCK_DIR="$OUT_ROOT/.v52_run.lock"
...
CKPT="$OUT_ROOT/train/bdse_v52_bfar_dbap.pt"
BEST_CKPT="$OUT_ROOT/train/bdse_v52_bfar_dbap.best.pt"
LATEST_CKPT="$OUT_ROOT/train/bdse_v52_bfar_dbap.latest.pt"
...
for path in dict.fromkeys(candidates):
    ...
    try:
        ...
        print(f"[v52] ignore unreadable checkpoint {path}: {type(exc).__name__}: {exc}", file=sys.stderr)
        continue
...
train_2gpu() {
  ...
  if [[ -n "$resume_checkpoint" ]]; then
    ...
    echo "[v52] resume checkpoint=$resume_checkpoint"
  elif [[ "$INIT_MODE" == "warm_start" ]]; then
    check_checkpoint "$V30_CKPT_IN"
    checkpoint_args+=(--warm-start-from "$V30_CKPT_IN")
    echo "[v52] warm-start checkpoint=$V30_CKPT_IN"
  else
    echo "[v52] training from random initialization (INIT_MODE=scratch)"
  fi
  echo "[v52] DDP training on physical GPUs $GPU0,$GPU1"
  echo "[v52] batch_per_gpu=$BATCH_SIZE_PER_GPU global_batch=$effective_global_batch workers_per_gpu=$NUM_WORKERS_PER_GPU"
  echo "[v52] train_config=$TRAIN_CONFIG val_split=$VAL_SPLIT val_scenarios=$VAL_SCENARIOS val_every=$VAL_EVERY_N_EPOCHS dense_val=$VAL_DENSE_DIAGNOSTIC"
  echo "[v52] auto_resume=$AUTO_RESUME save_every_n_steps=$SAVE_EVERY_N_STEPS init_mode=$INIT_MODE"
  echo "[v52] exact_selector_cpu_backend=$EXACT_SELECTOR_CPU_BACKEND workers_per_rank=$EXACT_SELECTOR_WORKERS_PER_RANK"
  echo "[v52-bfar-dbap] boundary-focused pairs with periodic exact AOCC supervision"
  ...
  torchrun \
    ...
    -m bdse.experiments.train \
      ...
      --log-file "$OUT_ROOT/train/bdse_v52_bfar_dbap.train_log.jsonl" \
      --output "$CKPT" \
    2>&1 | tee -a "$OUT_ROOT/logs/train_2gpu.out"
}

train_foundation_2gpu() {
  local foundation_ckpt="$FOUNDATION_OUT_ROOT/train/bdse_v52_factorized_anchor.pt"
  local foundation_best="$FOUNDATION_OUT_ROOT/train/bdse_v52_factorized_anchor.best.pt"
  mkdir -p "$FOUNDATION_OUT_ROOT/train" "$FOUNDATION_OUT_ROOT/logs"
  if [[ "$AUTO_RESUME" != "0" && -s "$foundation_best" && -s "$foundation_ckpt" ]]; then
    echo "[v52-anchor] reuse rebuilt foundation checkpoint=$foundation_best"
    return 0
  fi
  ...
  if [[ "$AUTO_RESUME" != "0" ]]; then
    ...
    foundation_resume="$(discover_latest_checkpoint \
      "$FOUNDATION_OUT_ROOT/train/bdse_v52_factorized_anchor.latest.pt" \
      "$FOUNDATION_OUT_ROOT/train/checkpoints" \
      "$foundation_ckpt")"
    if [[ -n "$foundation_resume" ]]; then
      foundation_checkpoint_args+=(--resume-from "$foundation_resume")
      echo "[v52-anchor] resume checkpoint=$foundation_resume"
    fi
  fi
  if [[ ${#foundation_checkpoint_args[@]} -eq 0 ]]; then
    echo "[v52-anchor] train factorized base+local anchor from random initialization"
  fi
  echo "[v52-anchor] config=$FOUNDATION_CONFIG output=$foundation_ckpt"
  CUDA_VISIBLE_DEVICES="$GPUS" \
  torchrun \
    --standalone \
    --nnodes=1 \
    ...
    -m bdse.experiments.train \
      ...
      --log-file "$FOUNDATION_OUT_ROOT/train/bdse_v52_factorized_anchor.train_log.jsonl" \
  ...
  if [[ ! -s "$foundation_best" && -s "$foundation_ckpt" ]]; then
    echo "[v52-anchor] warning: metric-specific best was not emitted; use the clean final checkpoint" >&2
    cp -f "$foundation_ckpt" "$foundation_best"
  fi
  check_checkpoint "$foundation_best"
  echo "[v52-anchor] rebuilt best checkpoint=$foundation_best"
}
...
run_open_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_open_loop \
    ...
    --device cuda \
    --output "$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap.gpu${shard_id}.json" \
    --per-sample-output "$OUT_ROOT/open_loop/open_loop_v52_bfar_dbap.gpu${shard_id}.jsonl" \
...
summary_paths = [root / f"open_loop_v52_bfar_dbap.gpu{i}.json" for i in range(2)]
row_paths = [root / f"open_loop_v52_bfar_dbap.gpu{i}.jsonl" for i in range(2)]
...
(root / "open_loop_v52_bfar_dbap.json").write_text(json.dumps(out, indent=2, sort_keys=True), encoding="utf-8")
with (root / "open_loop_v52_bfar_dbap.jsonl").open("w", encoding="utf-8") as f:
    for row in rows:
...
open_loop_2gpu() {
  local checkpoint="${V52_CKPT:-$BEST_CKPT}"
  check_checkpoint "$checkpoint"
  local shard_root="$OUT_ROOT/open_loop/.two_gpu_shards"
  prepare_open_loop_shards "$shard_root"

  echo "[v52] Open-loop source_split=$OPEN_LOOP_SPLIT across physical GPUs $GPU0 and $GPU1"
...
run_closed_loop_shard() {
  ...
  python -m bdse.experiments.evaluate_closed_loop \
    ...
    --experiment-uid "v52_bfar_dbap_${limit}_gpu${shard_id}" \
...
closed_loop_2gpu() {
  ...
  local run_root="$OUT_ROOT/closed_loop/v52_bfar_dbap_${limit}"
  local token_root="$run_root/scenario_token_shards"
  rm -rf "$run_root"
  mkdir -p "$run_root"
  prepare_closed_loop_token_shards "$limit" "$token_root"

  echo "[v52] Closed-loop token_source_split=$CL_TOKEN_SPLIT, $limit scenarios across physical GPUs $GPU0 and $GPU1"
  echo "[v52] Reuse $token_root/scenario_tokens_all.json for every compared method to keep the CL subset identical."
  local start_time end_time
  start_time=$(date +%s)
