...
# Accept both `SKIP_TRAIN=1 bash run_v32_cavr.sh` and
# `bash run_v32_cavr.sh SKIP_TRAIN=1` to avoid accidental training.
for kv in "$@"; do
  if [[ "$kv" == *=* ]]; then export "$kv"; fi
done


# v32 / CA-VR: Receding-Horizon Viability-Calibrated Decision-Sufficient Recovery
#   1) near-term hard / longer-term soft receding-horizon runtime constraints;
#   2) scene-adaptive component-risk normalization;
#   3) true epsilon-Pareto viability frontier over agent, route, soft risk,
#      BDSE certificate loss, and progress loss;
#   4) fixed evidence budget: recovery reuses the tournament score and does not
#      query future labels or additional evidence.
...
OUT_ROOT=${OUT_ROOT:-$ROOT_DIR/outputs_v32}
...
TRAIN_OUTPUT="$TRAIN_ROOT/bdse_v32_cavr.pt"
V32_CKPT=${V32_CKPT:-$TRAIN_ROOT/bdse_v32_cavr.best.pt}

pick_warm_start() {
  if [[ -f "$V30_CKPT_IN" ]]; then echo "$V30_CKPT_IN"; return; fi
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  if [[ -z "$WARM_START" ]]; then
    echo "Missing warm-start checkpoint. Set V30_CKPT_IN, V25_CKPT_IN, V24_CKPT_IN, or V11_CKPT." >&2
    exit 1
  fi
  echo "[train] v32 CA-VR finetune from $WARM_START"
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    --config bdse/configs/v32_bdse_cavr_train.yaml \
    --split train \
    ...
    --best-metrics fixed_budget_critical_score bdse_score teacher_action_match budget_vs_full_match selected_interaction_decisive_recall selected_hard_decisive_recall fallback_would_trigger_rate teacher_regret \
    --warm-start-from "$WARM_START" \
    --output "$TRAIN_OUTPUT" \
    --amp \
    --log-file "$LOG_ROOT/v32_cavr_train.jsonl" \
    > "$LOG_ROOT/v32_cavr_train.out" 2>&1
else
  echo "[train] SKIP_TRAIN=1, using existing checkpoint: $V32_CKPT"
fi

# For the causal runtime-only ablation, reuse the exact v30 checkpoint with v32 runtime guard/configs:
#   SKIP_TRAIN=1 V32_CKPT=outputs_v30/train/bdse_v30_pmvrbsr.best.pt OUT_ROOT=outputs_v32_runtime_v30ckpt bash run_v32_cavr.sh
if [[ ! -f "$V32_CKPT" ]]; then
  echo "Missing v32 checkpoint: $V32_CKPT" >&2
  echo "For runtime-only check, run: SKIP_TRAIN=1 V32_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v32_runtime_v30ckpt bash run_v32_cavr.sh" >&2
  exit 1
fi
...
paths = [
    ('v32_cavr_fixed_budget', root / 'open_loop_v32_cavr_fixed_budget.json'),
    ('v32_cavr_safety_fallback', root / 'open_loop_v32_cavr_safety_fallback.json'),
    ('v32_cavr_bbr_scur', root / 'open_loop_v32_cavr_bbr_scur.json'),
    ('v32_cavr_lcb_control', root / 'open_loop_v32_cavr_lcb_control.json'),
]
keys = [
...
if [[ "${SKIP_OPEN_LOOP:-0}" != "1" ]]; then
  if [[ "${OPEN_PARALLEL4:-1}" == "1" ]]; then
    run_open_loop "${GPU_OL_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-0}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" &
    run_open_loop "${GPU_OL_BBR:-1}" v32_cavr_bbr_scur bdse/configs/v32_bdse_cavr_bbr_scur_fast_cl.yaml "$V32_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v32_cavr_lcb_control bdse/configs/v32_bdse_cavr_lcb_control_fast_cl.yaml "$V32_CKPT" &
    wait
  else
    run_open_loop "${GPU_OL_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-1}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" &
    wait
    run_open_loop "${GPU_OL_BBR:-0}" v32_cavr_bbr_scur bdse/configs/v32_bdse_cavr_bbr_scur_fast_cl.yaml "$V32_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v32_cavr_lcb_control bdse/configs/v32_bdse_cavr_lcb_control_fast_cl.yaml "$V32_CKPT" &
    wait
  fi
  print_open_loop_compare
...
run_closed_loop() {
  ...
  (
    ...
    if [[ ! -s "$BDSE_CLOSED_LOOP_DIAG" ]]; then
      echo "ERROR: missing closed-loop diagnostics: $BDSE_CLOSED_LOOP_DIAG" >&2
      exit 3
    fi
  )
}
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  if [[ "${CL_PARALLEL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-0}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" 20 &
    run_closed_loop "${GPU_CL_BBR:-1}" v32_cavr_bbr_scur bdse/configs/v32_bdse_cavr_bbr_scur_fast_cl.yaml "$V32_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v32_cavr_lcb_control bdse/configs/v32_bdse_cavr_lcb_control_fast_cl.yaml "$V32_CKPT" 20 &
    wait
  else
    run_closed_loop "${GPU_CL_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-1}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" 20 &
    wait
    run_closed_loop "${GPU_CL_BBR:-0}" v32_cavr_bbr_scur bdse/configs/v32_bdse_cavr_bbr_scur_fast_cl.yaml "$V32_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v32_cavr_lcb_control bdse/configs/v32_bdse_cavr_lcb_control_fast_cl.yaml "$V32_CKPT" 20 &
    wait
  fi
fi
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v32_cavr_fixed_budget_20" \
    "$CL_ROOT/v32_cavr_safety_fallback_20" \
    "$CL_ROOT/v32_cavr_bbr_scur_20" \
    "$CL_ROOT/v32_cavr_lcb_control_20" \
    --csv "$CL_ROOT/v32_20_compare.csv"
fi
...
for path in sorted(list(root.glob('*.closed_loop_20.diag.jsonl')) + list(root.glob('*.closed_loop_50.diag.jsonl'))):
    ...
    for line in path.read_text(errors='ignore').splitlines():
        ...
        selected_flags += int(bool(t.get('final_action_safety_flag', t.get('selected_action_safety_flag', False))))
    ...
    print(path.name, 'replans=', len(replans), 'fallback_triggered_rate=', round(triggered, 4),
          'final_action_safety_flag_rate=', round(selected_flags / len(replans), 4),
...
if [[ "${RUN_CL50:-0}" == "1" ]]; then
  if [[ "${RUN_CL50_ALL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL50_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-0}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" 50 &
    run_closed_loop "${GPU_CL50_BBR:-1}" v32_cavr_bbr_scur bdse/configs/v32_bdse_cavr_bbr_scur_fast_cl.yaml "$V32_CKPT" 50 &
    run_closed_loop "${GPU_CL50_LCB:-1}" v32_cavr_lcb_control bdse/configs/v32_bdse_cavr_lcb_control_fast_cl.yaml "$V32_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v32_cavr_fixed_budget_50" \
      "$CL_ROOT/v32_cavr_safety_fallback_50" \
      "$CL_ROOT/v32_cavr_bbr_scur_50" \
      "$CL_ROOT/v32_cavr_lcb_control_50" \
      --csv "$CL_ROOT/v32_50_compare.csv"
  else
    run_closed_loop "${GPU_CL50_FIXED:-0}" v32_cavr_fixed_budget bdse/configs/v32_bdse_cavr_fixed_budget_fast_cl.yaml "$V32_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-1}" v32_cavr_safety_fallback bdse/configs/v32_bdse_cavr_safety_fallback_fast_cl.yaml "$V32_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v32_cavr_fixed_budget_50" \
      "$CL_ROOT/v32_cavr_safety_fallback_50" \
...
echo "  $TRAIN_ROOT/bdse_v32_cavr.best.pt"
echo "  $OPEN_ROOT/open_loop_v32_*.json"
echo "  $CL_ROOT/v32_20_compare.csv"
echo "  $LOG_ROOT/*.closed_loop_20.diag.jsonl"
echo "Runtime-only first check: export SKIP_TRAIN=1 V32_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v32_runtime_v30ckpt RUN_MODE=open_loop; bash run_v32_cavr.sh"
echo "Runtime-only CL20: export SKIP_TRAIN=1 V32_CKPT=$V30_CKPT_IN OUT_ROOT=outputs_v32_runtime_v30ckpt RUN_MODE=cl20; bash run_v32_cavr.sh"
echo "Finetune: export V30_CKPT_IN=outputs_v30/train/bdse_v30_pmvrbsr.best.pt OUT_ROOT=outputs_v32 TRAIN_MAX_SCENARIOS=12000 VAL_MAX_SCENARIOS=1000 TRAIN_EPOCHS=3 RUN_MODE=all; bash run_v32_cavr.sh"

# CL50 after finetune: export SKIP_TRAIN=1 V32_CKPT=outputs_v32/train/bdse_v32_cavr.best.pt RUN_MODE=cl50; bash run_v32_cavr.sh
