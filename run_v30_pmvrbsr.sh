...
# Accept both `SKIP_TRAIN=1 bash run_v30_pmvrbsr.sh` and
# `bash run_v30_pmvrbsr.sh SKIP_TRAIN=1` to avoid accidental training.
for kv in "$@"; do
  if [[ "$kv" == *=* ]]; then export "$kv"; fi
done


# v30 / PMV-RBSR: Pareto Min-Violation Risk-Bounded Safety Recovery
...
TRAIN_OUTPUT="$TRAIN_ROOT/bdse_v30_pmvrbsr.pt"
V30_CKPT=${V30_CKPT:-$TRAIN_ROOT/bdse_v30_pmvrbsr.best.pt}
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  echo "[train] v30 PMV-RBSR finetune from $WARM_START"
  torchrun --standalone --nproc_per_node=${NPROC_PER_NODE:-2} -m bdse.experiments.train \
    --config bdse/configs/v30_bdse_pmvrbsr_train.yaml \
    --split train \
    ...
    --output "$TRAIN_OUTPUT" \
    --amp \
    --log-file "$LOG_ROOT/v30_pmvrbsr_train.jsonl" \
    > "$LOG_ROOT/v30_pmvrbsr_train.out" 2>&1
else
  echo "[train] SKIP_TRAIN=1, using existing checkpoint: $V30_CKPT"
fi

# For zero-training ablation, reuse the v25/v26 checkpoint with v30 runtime guard/configs:
#   SKIP_TRAIN=1 V30_CKPT=outputs_v25/train/bdse_v25_dgcace.best.pt bash run_v30_pmvrbsr.sh
if [[ ! -f "$V30_CKPT" ]]; then
  echo "Missing v30 checkpoint: $V30_CKPT" >&2
  echo "For runtime-only check, run: SKIP_TRAIN=1 V30_CKPT=$V25_CKPT_IN bash run_v30_pmvrbsr.sh" >&2
  exit 1
fi
...
paths = [
    ('v30_pmvrbsr_fixed_budget', root / 'open_loop_v30_pmvrbsr_fixed_budget.json'),
    ('v30_pmvrbsr_safety_fallback', root / 'open_loop_v30_pmvrbsr_safety_fallback.json'),
    ('v30_pmvrbsr_bbr_scur', root / 'open_loop_v30_pmvrbsr_bbr_scur.json'),
    ('v30_pmvrbsr_lcb_control', root / 'open_loop_v30_pmvrbsr_lcb_control.json'),
]
keys = [
...
if [[ "${SKIP_OPEN_LOOP:-0}" != "1" ]]; then
  if [[ "${OPEN_PARALLEL4:-1}" == "1" ]]; then
    run_open_loop "${GPU_OL_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-0}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" &
    run_open_loop "${GPU_OL_BBR:-1}" v30_pmvrbsr_bbr_scur bdse/configs/v30_bdse_pmvrbsr_bbr_scur_fast_cl.yaml "$V30_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v30_pmvrbsr_lcb_control bdse/configs/v30_bdse_pmvrbsr_lcb_control_fast_cl.yaml "$V30_CKPT" &
    wait
  else
    run_open_loop "${GPU_OL_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" &
    run_open_loop "${GPU_OL_SAFETY:-1}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" &
    wait
    run_open_loop "${GPU_OL_BBR:-0}" v30_pmvrbsr_bbr_scur bdse/configs/v30_bdse_pmvrbsr_bbr_scur_fast_cl.yaml "$V30_CKPT" &
    run_open_loop "${GPU_OL_LCB:-1}" v30_pmvrbsr_lcb_control bdse/configs/v30_bdse_pmvrbsr_lcb_control_fast_cl.yaml "$V30_CKPT" &
    wait
  fi
  print_open_loop_compare
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  if [[ "${CL_PARALLEL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-0}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" 20 &
    run_closed_loop "${GPU_CL_BBR:-1}" v30_pmvrbsr_bbr_scur bdse/configs/v30_bdse_pmvrbsr_bbr_scur_fast_cl.yaml "$V30_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v30_pmvrbsr_lcb_control bdse/configs/v30_bdse_pmvrbsr_lcb_control_fast_cl.yaml "$V30_CKPT" 20 &
    wait
  else
    run_closed_loop "${GPU_CL_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" 20 &
    run_closed_loop "${GPU_CL_SAFETY:-1}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" 20 &
    wait
    run_closed_loop "${GPU_CL_BBR:-0}" v30_pmvrbsr_bbr_scur bdse/configs/v30_bdse_pmvrbsr_bbr_scur_fast_cl.yaml "$V30_CKPT" 20 &
    run_closed_loop "${GPU_CL_LCB:-1}" v30_pmvrbsr_lcb_control bdse/configs/v30_bdse_pmvrbsr_lcb_control_fast_cl.yaml "$V30_CKPT" 20 &
    wait
  fi
fi
...
if [[ "${RUN_CL20:-1}" == "1" ]]; then
  python -m bdse.tools.collect_closed_loop_metrics \
    "$CL_ROOT/v30_pmvrbsr_fixed_budget_20" \
    "$CL_ROOT/v30_pmvrbsr_safety_fallback_20" \
    "$CL_ROOT/v30_pmvrbsr_bbr_scur_20" \
    "$CL_ROOT/v30_pmvrbsr_lcb_control_20" \
    --csv "$CL_ROOT/v30_20_compare.csv"
fi
...
if [[ "${RUN_CL50:-0}" == "1" ]]; then
  if [[ "${RUN_CL50_ALL4:-1}" == "1" ]]; then
    run_closed_loop "${GPU_CL50_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-0}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" 50 &
    run_closed_loop "${GPU_CL50_BBR:-1}" v30_pmvrbsr_bbr_scur bdse/configs/v30_bdse_pmvrbsr_bbr_scur_fast_cl.yaml "$V30_CKPT" 50 &
    run_closed_loop "${GPU_CL50_LCB:-1}" v30_pmvrbsr_lcb_control bdse/configs/v30_bdse_pmvrbsr_lcb_control_fast_cl.yaml "$V30_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v30_pmvrbsr_fixed_budget_50" \
      "$CL_ROOT/v30_pmvrbsr_safety_fallback_50" \
      "$CL_ROOT/v30_pmvrbsr_bbr_scur_50" \
      "$CL_ROOT/v30_pmvrbsr_lcb_control_50" \
      --csv "$CL_ROOT/v30_50_compare.csv"
  else
    run_closed_loop "${GPU_CL50_FIXED:-0}" v30_pmvrbsr_fixed_budget bdse/configs/v30_bdse_pmvrbsr_fixed_budget_fast_cl.yaml "$V30_CKPT" 50 &
    run_closed_loop "${GPU_CL50_SAFETY:-1}" v30_pmvrbsr_safety_fallback bdse/configs/v30_bdse_pmvrbsr_safety_fallback_fast_cl.yaml "$V30_CKPT" 50 &
    wait
    python -m bdse.tools.collect_closed_loop_metrics \
      "$CL_ROOT/v30_pmvrbsr_fixed_budget_50" \
      "$CL_ROOT/v30_pmvrbsr_safety_fallback_50" \
...
echo "  $TRAIN_ROOT/bdse_v30_pmvrbsr.best.pt"
echo "  $OPEN_ROOT/open_loop_v30_*.json"
echo "  $CL_ROOT/v30_20_compare.csv"
echo "  $LOG_ROOT/*.closed_loop_20.diag.jsonl"
echo "Runtime-only first check: export SKIP_TRAIN=1 V30_CKPT=$V25_CKPT_IN RUN_MODE=open_loop; bash run_v30_pmvrbsr.sh"
echo "Runtime-only CL20: export SKIP_TRAIN=1 V30_CKPT=$V25_CKPT_IN RUN_MODE=cl20; bash run_v30_pmvrbsr.sh"
echo "Finetune: export TRAIN_MAX_SCENARIOS=12000 VAL_MAX_SCENARIOS=1000 TRAIN_EPOCHS=5 RUN_MODE=all; bash run_v30_pmvrbsr.sh"

# CL50 after finetune: export SKIP_TRAIN=1 V30_CKPT=outputs_v30/train/bdse_v30_pmvrbsr.best.pt RUN_MODE=cl50; bash run_v30_pmvrbsr.sh
