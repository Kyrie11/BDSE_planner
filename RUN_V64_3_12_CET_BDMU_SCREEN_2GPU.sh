...
if not r.get('full_promotion', False):
    raise SystemExit('V64.3.7 DARM+DBR full causal/value gate is not promoted; CET-BDMU is blocked.')
...
# V64.3.12 requires fixed-reference C1-M/C2-M/C1-B/C2-B validation metrics in addition to
# normal open-loop endpoint metrics.  The checker refuses moving-target-only logs.
...
export TRAIN_CONFIG="bdse/configs/v64_3_12_cc_aocc_cet_bdmu_daepc_screen_2gpu.yaml"
export EVAL_CONFIG="bdse/configs/v64_3_12_cc_aocc_cet_bdmu_cl.yaml"
export OUT_ROOT="${OUT_ROOT:-outputs_v64_3_12_cet_bdmu_screen_2gpu_v1}"
...
python -m bdse.tools.check_v64_3_12_ret_cet_bdmu_screen \
  ...
  --output "$OUT_ROOT/provenance/v64_3_12_cet_bdmu_screen.json" \
  --variant cet
echo "[v64.3.12] screen report: $OUT_ROOT/provenance/v64_3_12_cet_bdmu_screen.json"
